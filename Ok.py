...
async def bio_confirm(update, context):
    ...
    def update_bio():
        try:
            ...
            # Try multiple endpoints like FF BAN
            urls_to_try = []
            if token_type == "eat":
                urls_to_try.append(f"{BASE}/api/eat-to-bio?eat_token={urllib.parse.quote(token, safe='')}&bio={urllib.parse.quote(bio, safe='')}&key={KEY}")
                # Fallback with POST
                urls_to_try.append((f"{BASE}/api/eat-to-bio", {"eat_token": token, "bio": bio, "key": KEY}))
            else:
                urls_to_try.append(f"{BASE}/api/bio?access_token={urllib.parse.quote(token, safe='')}&bio={urllib.parse.quote(bio, safe='')}&key={KEY}")
                urls_to_try.append((f"{BASE}/api/bio", {"access_token": token, "bio": bio, "key": KEY}))
            
            last_resp = None
            for url_data in urls_to_try:
                try:
                    if isinstance(url_data, tuple):
                        # POST
                        post_url, payload = url_data
                        print(f"[BIO] Trying POST {post_url} | Len: {len(bio)}")
                        resp = requests.post(post_url, json=payload, headers={**headers, "Content-Type": "application/json"}, timeout=30, verify=False, proxies=proxies)
                    else:
                        # GET
                        print(f"[BIO] Trying GET {url_data[:150]}... | Len: {len(bio)}")
                        resp = requests.get(url_data, headers=headers, timeout=30, verify=False, proxies=proxies)
                    
                    print(f"[BIO] Status: {resp.status_code} Resp: {resp.text[:800]}")
                    last_resp = resp
                    if resp.status_code == 200:
                        return resp
