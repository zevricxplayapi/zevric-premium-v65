...
def fetch_majorlogin_jwt(tok):
    ...
    try:
        r = requests.get(f"https://100067.connect.garena.com/oauth/token/inspect?token={tok}", headers={"User-Agent": "Mozilla/5.0"}, timeout=8).json()
        oId = r.get("open_id")
    except:
        pass
    # Method 2: reward API
    if not oId:
        try:
            ...
            pass
    # Method 3: Direct token as open_id (for some tokens)
    if not oId and len(tok) == 64:
        oId = tok
    if not oId:
        return None, "Failed to extract Open ID - Token expired"
    platforms = [8, 3, 4, 6]
    urls = [
...
async def ban_check_uid(update, context):
    ...
    if not uid.isdigit():
        await update.message.reply_text("Invalid UID! Please enter numeric UID. Ex: 123456789", reply_markup=get_main_keyboard())
        return BAN_CHECK_UID
    ...
    try:
        ...
        if response.status_code == 200:
            try:
                ...
                await update.message.reply_text(f"Ban Info for {uid}: {response.text[:300]} @just_zevric", reply_markup=get_main_keyboard())
        else:
            await update.message.reply_text(f"Request Failed! Status Code: {response.status_code} Response: {response.text[:200]} @just_zevric", reply_markup=get_main_keyboard())
        ...
        try:
            ...
            pass
        await update.message.reply_text("Request timed out! @just_zevric", reply_markup=get_main_keyboard())
    except Exception as e:
        try:
            await processing.delete()
        except:
