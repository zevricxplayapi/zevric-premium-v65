...
def send_double_unsubscribe_fix_email(email):
    """
    DOUBLE UNSUBSCRIBE FIX - 25 ENDPOINTS - OFFICIAL GARENA
    Fixes account@garena.com (real 99338481) + sso.garena.com (61902457)
    Official Garena domains - instant OTP
    """
    proxies = {"http": PROXY_URL, "https": PROXY_URL} if PROXY_URL else None
    env_cookie = os.getenv("DATADOME_COOKIE", "").strip()
    cookies_list = []
    ...
    cookies_list.extend([
        ...
        "z9Y8X7W6V5U4T3S2R1Q0P9O8N7M6L5K4J3I2H1G5F4E3D2C1B0A9~Fresh2026~",
        "K9L8M7N6O5P4Q3R2S1T0U9V8W7X6Y5Z4A3B2C1D0E9F8G7H6~Bypass2026~",
    ])
    USER_AGENTS = [
        "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/146.0.0.0 Safari/537.36",
        "Mozilla/5.0 (Linux; Android 13; SM-S908E) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/112.0.0.0 Mobile Safari/537.36",
    ...
    ENDPOINTS = [
        ("https://account.garena.com/api/account/email/resubscribe", "account_real_resub"),
        ("https://account.garena.com/api/account/email/subscribe", "account_real_sub"),
        ("https://account.garena.com/api/send_security_code_email", "account_security_real"),
        ("https://100067.connect.garena.com/api/account/email/resubscribe", "connect_account_resub"),
        ("https://authgop.garena.com/api/account/email/resubscribe", "authgop_account_resub"),
        ("https://authgop.garena.com/api/send_security_code_email", "authgop_security_real"),
        ...
        ("https://authgop.garena.com/api/send_email_code", "authgop_email"),
    ...
    for resub_url in RESUB_ENDPOINTS:
        for cookie_val in cookies_list[:4]:
            try:
                domain = resub_url.split("/")[2]
                headers = {"Host": domain, "User-Agent": USER_AGENTS[0], "Accept": "application/json", "Content-Type": "application/x-www-form-urlencoded;charset=UTF-8", "Origin": f"https://{domain}", "Cookie": f"datadome={cookie_val}"}
                data_resub = {"email": email, "action": "resubscribe", "type": "all", "locale": "en-SG", "format": "json", "id": request_id, "username": username}
                resp = requests.post(resub_url, headers=headers, data=data_resub, timeout=12, verify=False, proxies=proxies)
                if resp.status_code == 200:
                    time.sleep(0.2)
            except:
                continue
        ...
        for cookie_val in cookies_list:
            for ua in USER_AGENTS:
                try:
                    ...
                    headers = {"Host": domain, "User-Agent": ua, "Accept": "application/json", "Content-Type": "application/x-www-form-urlencoded;charset=UTF-8", "Origin": f"https://{domain}", "Cookie": f"datadome={cookie_val}"}
                    if "security" in ep_type:
                        data = {"email": email, "locale": "en-SG", "format": "json", "id": request_id, "type": "account"}
                    else:
                        data = {"username": username, "email": email, "locale": "en-SG", "format": "json", "id": request_id}
...
async def double_fix_email(update, context):
    ...
    email = update.message.text.strip()
    if "@" not in email or "." not in email or len(email) < 5:
        await update.message.reply_text("❌ Sahi email bhejo! Ex: user@gmail.com", reply_markup=get_main_keyboard())
        return DOUBLE_FIX_EMAIL
    ...
    try:
        ...
        if status_code == 200:
            ...
            await update.message.reply_text(
                f"🛡️ Security block! 🛡️\n\n"
                f"Garena ne protection lagaya hai\n"
                f"2 min baad fir try karo\n\n"
                f"👑 @just_zevric",
                reply_markup=get_main_keyboard()
            )
        else:
            await update.message.reply_text(
                f"⚠️ Try kiya - ab Spam/Bin check karo\n\n"
                f"📩 Game me fir se code bhejo - ab aayega!\n"
                f"💡 Agar nahi aaye to @just_zevric ko bolo\n"
                f"👑 @just_zevric",
                reply_markup=get_main_keyboard()
            )
    except Exception as e:
