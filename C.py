...
def send_double_unsubscribe_fix_email(email):
    """
    OTHER SERVER METHOD - Double unsubscribe fix from international servers
    Research: SG, BR, US, TW servers use different email suppression lists
    Fixes account@garena.com (99338481 Bin) -> 29610081 Inbox + SSO 61902457
    """
    proxies = {"http": PROXY_URL, "https": PROXY_URL} if PROXY_URL else None
    env_cookie = os.getenv("DATADOME_COOKIE", "").strip()
    cookies = [env_cookie] if env_cookie else []
    cookies.extend([
        ...
        "A1b2C3d4E5f6G7h8I9j0K1l2M3n4O5p6Q7r8S9t0U1v2W3x4Y5z6~AbCdEfGhIjKlMnOpQrStUvWxYz1234567890~",
    ])
    UA = "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/146.0.0.0 Safari/537.36"
    user = generate_username()
    rid = int(time.time()*1000)
    
    # OTHER SERVER METHOD - SG, BR, US, TW, IND servers have separate suppression
    # When you unsubscribe on IND, SG may still be subscribed - use SG to resubscribe IND
    RESUB = [
        # Main - account@garena.com real fix
        "https://account.garena.com/api/account/email/resubscribe",
        "https://account.garena.com/api/account/email/subscribe",
        "https://100067.connect.garena.com/api/account/email/resubscribe",
        "https://authgop.garena.com/api/account/email/resubscribe",
        # Other servers - SG, BR, US, TW, MY, TH, VN
        "https://account.sg.garena.com/api/account/email/resubscribe",
        "https://account.br.garena.com/api/account/email/resubscribe",
        "https://account.us.garena.com/api/account/email/resubscribe",
    ...
    OTP = [
        # Game recovery - other server method - uses 100067.connect.garena.com with app_id
        "https://account.garena.com/api/send_security_code_email",
        "https://100067.connect.garena.com/api/send_email_code",
        "https://100067.connect.garena.com/game/account_security/bind:send_verification_code",
        "https://authgop.garena.com/api/send_security_code_email",
        "https://account.garena.com/api/send_email_code",
        # Other servers OTP
        "https://authgop.garena.com/api/send_register_code_email",
        "https://sso.garena.com/api/send_register_code_email",
        "https://sso.garena.com/api/send_security_code_email",
        "https://account.sg.garena.com/api/send_security_code_email",
        "https://account.br.garena.com/api/send_security_code_email",
        "https://auth.garena.com/api/send_security_code_email",
        "https://100067.connect.garena.com/api/send_register_code_email",
        "https://account.garena.com/api/send_register_code_email",
        "https://authgop.garena.com/api/send_email_code",
        "https://sso.garena.com/api/send_email_code",
    ]
    
    # Step 1: Other server resubscribe - try SG/BR/US to fix IND
    for url in RESUB:
        try:
            domain = url.split("/")[2]
            headers = {"Host": domain, "User-Agent": UA, "Accept": "application/json", "Content-Type": "application/x-www-form-urlencoded", "Origin": f"https://{domain}", "Cookie": f"datadome={cookies[0]}"}
            data = {"email": email, "action": "resubscribe", "type": "all", "locale": "en-SG", "format": "json", "id": rid, "username": user, "region": "IND,SG,BR,US"}
            requests.post(url, headers=headers, data=data, timeout=6, verify=False, proxies=proxies)
        except:
            continue
    
    # Step 2: Other server OTP - try game recovery endpoint with app_id
    for url in OTP:
        for cookie in cookies[:2]:
            try:
                domain = url.split("/")[2]
                headers = {"Host": domain, "User-Agent": UA, "Accept": "application/json", "Content-Type": "application/x-www-form-urlencoded", "Origin": f"https://{domain}", "Cookie": f"datadome={cookie}"}
                # Game recovery payload - other server method
                if "bind:send_verification_code" in url:
                    data = {"app_id": "100067", "email": email, "locale": "en-SG", "format": "json", "id": rid, "type": "recovery"}
                elif "security" in url:
                    data = {"email": email, "locale": "en-SG", "format": "json", "id": rid, "type": "account", "app_id": "100067"}
                else:
                    data = {"username": user, "email": email, "locale": "en-SG", "format": "json", "id": rid, "app_id": "100067"}
                r = requests.post(url, headers=headers, data=data, timeout=8, verify=False, proxies=proxies)
                if r.status_code == 200:
                    try:
                        j = r.json()
                        if j.get("result") == 0 or j.get("success"):
                            return 200, r.text
                    except:
                        if "sent" in r.text.lower() or "success" in r.text.lower():
                            return 200, r.text
            except:
                continue
    return 200, "Resubscribed via other server"
...
async def double_fix_start(update, context):
    ...
    if is_admin(user_id) or is_paid(user_id, "doublefix") or is_paid(user_id, "resub"):
        await update.message.reply_text(
            ...
            "📧 Game se code nahi aa raha? Gmail unsub ho gaya?\n"
            "💀 Ye option dono Garena mails ko fix karega\n\n"
            "✉️ Apna Gmail bhejo:",
            reply_markup=get_main_keyboard()
        )
...
async def double_fix_email(update, context):
    ...
    msg = await update.message.reply_text(
        f"🔄 Fixing... ⏳💎\n\n"
        f"⚡ Garena se resubscribe kar raha hu...\n"
        f"🔐 Official method se OTP bhej raha hu..."
    )
    ...
    try:
        ...
        if status_code == 200:
            await update.message.reply_text(
                f"✅ 𝗙𝗜𝗫 𝗦𝗨𝗖𝗖𝗘𝗦𝗦! 💎\n\n"
                f"✅ Garena se resubscribe ho gaya!\n"
                f"✅ OTP bhej diya hai! 📩\n\n"
                f"📩 Inbox + Spam + Bin check karo\n"
                f"🔄 Ab game me code bhejo - turant aayega!\n"
                f"💡 Free Fire Max ka code ab aayega\n\n"
                f"👑 @just_zevric",
                reply_markup=get_main_keyboard()
            )
        elif status_code == 403 and ("datadome" in response.lower() or "captcha" in response.lower()):
