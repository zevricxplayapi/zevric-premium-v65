...
async def bio_text(update, context):
    ...
    # Long bio support - allow up to 1000 chars
    if len(bio_raw) < 1 or len(bio_raw) > 1000:
        await update.message.reply_text(f"❌ 1-1000 chars! Now {len(bio_raw)}\n💡 Long bio supported, try shorter", reply_markup=get_main_keyboard())
        return BIO_TEXT
...
async def bio_confirm(update, context):
    ...
    msg = await update.message.reply_text(f"⏳ Updating Long Bio... 🚀💎\n📝 {bio[:80]} ✨\n🔑 Type: {token_type}\n📏 Length: {len(bio)} chars", reply_markup=get_main_keyboard())
    ...
    def update_bio():
        try:
            ...
            # FF BAN STYLE - Try POST first (supports long bio), then GET fallback
            # POST is better for long bio (URL length limit bypass)
            if token_type == "eat":
                api_url = f"{BASE}/api/eat-to-bio"
                payload = {"eat_token": token, "bio": bio, "key": KEY}
                # Also try GET version as fallback
                get_url = f"{BASE}/api/eat-to-bio?eat_token={urllib.parse.quote(token, safe='')}&bio={urllib.parse.quote(bio, safe='')}&key={KEY}"
            else:
                api_url = f"{BASE}/api/bio"
                payload = {"access_token": token, "bio": bio, "key": KEY}
                get_url = f"{BASE}/api/bio?access_token={urllib.parse.quote(token, safe='')}&bio={urllib.parse.quote(bio, safe='')}&key={KEY}"
            
            print(f"[LONG BIO] Trying POST {api_url} | Type: {token_type} | Len: {len(bio)}")
            
            # Try POST first - supports long bio
            try:
                resp = requests.post(api_url, json=payload, headers=headers, timeout=30, verify=False, proxies=proxies)
                print(f"[LONG BIO] POST Status: {resp.status_code} Resp: {resp.text[:500]}")
                if resp.status_code == 200:
                    return resp
            except Exception as e:
                print(f"[LONG BIO] POST Error: {e}")
            
            # Fallback to GET if POST fails
            print(f"[LONG BIO] POST failed, trying GET fallback...")
            try:
                headers_get = {"User-Agent": "Mozilla/5.0 (Linux; Android 13) AppleWebKit/537.36", "Accept": "application/json"}
                resp = requests.get(get_url, headers=headers_get, timeout=25, verify=False, proxies=proxies)
                print(f"[LONG BIO] GET Status: {resp.status_code} Resp: {resp.text[:500]}")
                return resp
            except Exception as e:
                print(f"[LONG BIO] GET Error: {e}")
                return None
                
        except Exception as e:
            print(f"[LONG BIO] Error: {e}")
            return None
    ...
    try:
        ...
        if resp and resp.status_code == 200:
            try:
                data = resp.json()
                if data.get("success") or data.get("result") == 0 or "success" in resp.text.lower() or "updated" in resp.text.lower() or "bio" in resp.text.lower():
                    await update.message.reply_text(f"🎉 𝗟𝗢𝗡𝗚 𝗕𝗜𝗢 𝗨𝗣𝗗𝗔𝗧𝗘𝗗! 🎉💎\n\n📝 Bio: {bio[:200]} 🔥\n✅ Type: {token_type.upper()}\n📏 Length: {len(bio)} chars\n✅ Wait 1-2 min in game, relogin karo! 🚀💎\n\n👑 @just_zevric", reply_markup=get_main_keyboard())
                else:
                    await update.message.reply_text(f"⚠️ API Response:\n{resp.text[:600]}\n\n💡 Token expire ho sakta hai, fresh token try karo\n@just_zevric", reply_markup=get_main_keyboard())
            except:
                if "success" in resp.text.lower() or "updated" in resp.text.lower() or resp.status_code == 200:
                    await update.message.reply_text(f"🎉 𝗟𝗢𝗡𝗚 𝗕𝗜𝗢 𝗨𝗣𝗗𝗔𝗧𝗘𝗗! 🎉💎\n📝 Bio: {bio[:200]} 🔥\n📏 {len(bio)} chars\n✅ Wait 2 min! 🚀💎\n👑 @just_zevric", reply_markup=get_main_keyboard())
                else:
                    await update.message.reply_text(f"⚠️ Response: {resp.text[:600]}\n@just_zevric", reply_markup=get_main_keyboard())
        else:
            status = resp.status_code if resp and hasattr(resp, 'status_code') else "No Response"
            text = resp.text[:600] if resp and hasattr(resp, 'text') else "No response - API down ya URL too long, POST try kar rahe hain"
            # If No Response, it's likely URL too long for GET, but we already tried POST
            if "No Response" in str(status):
                await update.message.reply_text(f"❌ FAILED! Status: {status}\nResp: {text}\n\n💡 Long Bio ke liye POST use kar rahe hain, API down ho sakta hai. 5 min baad try karo ya bio thoda chhota karo (500 chars)\n🔑 Fresh token/EAT try karo\n@just_zevric", reply_markup=get_main_keyboard())
            else:
                await update.message.reply_text(f"❌ FAILED! Status: {status}\nResp: {text}\n\n💡 Fresh token/EAT try karo\n@just_zevric", reply_markup=get_main_keyboard())
    except Exception as e:
        try:
