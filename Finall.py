...
API_URL = 'https://client.ind.freefiremobile.com/GetLoginData'  # FF Ban - from Subscribe_3.py Line 81
BODY_BASE64 = (
...
async def resubscribe_email(update, context):
    ...
    try:
        ...
        if 'captcha' in resp.text.lower() or 'geo.captcha' in resp.text.lower():
            await msg.edit_text(f"⚠️ Garena ne Captcha laga diya hai! 😔\n\n📧 {email} pe OTP nahi gaya. Garena website pe manually try karo.\n\n🔗 Details: {resp.text[:300]}\n\n👑 @just_zevric", reply_markup=get_main_keyboard())
            return ConversationHandler.END
        try:
            j = resp.json()
            if resp.status_code == 200 and j.get("result")==0:
                ...
                await msg.edit_text(f"❌ Failed! {j}")
        except:
            await msg.edit_text(f"📩 Response: {resp.text[:300]}")
    except Exception as e:
        await msg.edit_text(f"❌ Error: {e} 😔")
    return ConversationHandler.END
...
async def platform_token(update, context):
    ...
    try:
        ...
        for k in m:
            if k not in a:
                txt+=f"👑 𝐌𝐚𝐢𝐧 𝐏𝐥𝐚𝐭𝐟𝐨𝐫𝐦: {m.get(k, 'Unknown')} 💎\n"
                break
        txt+=f"\n👑 @just_zevric 💎"
        await msg.edit_text(txt)
    except Exception as e:
