...
👑 @just_zevric"""
    ...
    try:
        if os.path.exists(UPI_QR_PATH):
            await update.message.reply_photo(photo=open(UPI_QR_PATH, "rb"), caption=f"💳 UPI QR - ₹{price_inr}\nUPI ID: {UPI_ID}\nName: Zevric Play")
    except Exception as e:
        print(f"[PAY] UPI QR error: {e}")
    
    # Send USDT QR
    try:
        qr_path = USDT_QR_PATH if os.path.exists(USDT_QR_PATH) else USDT_QR_PATH2
        if os.path.exists(qr_path):
            await update.message.reply_photo(photo=open(qr_path, "rb"), caption=f"💰 USDT QR - ${price_usdt}\nAddr: {USDT_ADDR}\nNetwork: TRON (TRC20) Only!")
        elif os.path.exists(USDT_QR_PATH2):
            await update.message.reply_photo(photo=open(USDT_QR_PATH2, "rb"), caption=f"💰 USDT QR - ${price_usdt}\nAddr: {USDT_ADDR}")
    except Exception as e:
