...
BOT_TOKEN=os.getenv("TELEGRAM_BOT_TOKEN") or os.getenv("BOT_TOKEN") or os.getenv("TOKEN") or "YOUR_TOKEN"
if BOT_TOKEN == "YOUR_TOKEN":
    print("❌ CRITICAL: BOT_TOKEN missing! Set TELEGRAM_BOT_TOKEN in Railway Variables")
    print("Bot will run Flask only to avoid crash, but Telegram polling will not start until token is set")
...
# ========== LOG GROUP ID - Added as per user request ==========
...
async def send_log(context, text):
    """Send log to log group - non-blocking, ignore errors"""
    try:
        if LOG_CHAT_ID:
            # Trim to 4000 chars for telegram limit
            if len(text) > 4000:
                text = text[:4000]
            await context.bot.send_message(chat_id=LOG_CHAT_ID, text=f"📋 LOG:\n{text}")
    except Exception as e:
        print(f"[LOG] Failed to send to {LOG_CHAT_ID}: {e}")

def sync_log_print(msg):
    print(msg)
    # print will go to railway logs anyway
