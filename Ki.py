...
def send_register_code_email(email):
    ...
    try:
        response = requests.post("https://authgop.garena.com/api/send_register_code_email", headers=headers, data=data, timeout=20, proxies=proxies)
        print(f"[OTP] Status: {response.status_code} | Email: {email} | Resp: {response.text[:400]}")
