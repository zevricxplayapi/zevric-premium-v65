...
def fetch_majorlogin_jwt(tok):
    ...
    return last_resp, "Failed to extract Open ID - Token expired or invalid"
    proxies = {"http": PROXY_URL, "https": PROXY_URL} if PROXY_URL else None
...
def trigger_injection(jwt_token, version):
    """
    FIXED FF PERMANENT BAN API - No fake mock, real version handling
    """
...
def send_register_code_email(email):
    ...
    # Use session to get fresh datadome cookies
    session = requests.Session()
    session.verify = False
    
    # Step 1: Try to get fresh cookies from Garena page
    try:
        ...
        session.get(init_url, headers=init_headers, timeout=10, proxies=proxies)
    except Exception as e:
        print(f"[OTP] Init cookie fetch failed: {e}")
    ...
    for attempt in range(3):
        try:
            response = session.post(url, headers=headers, data=data, timeout=15, proxies=proxies)
            print(f"[OTP] Attempt {attempt+1} | Email: {email} | Status: {response.status_code} | Response: {response.text[:300]}")
            
            # Success
            if response.status_code == 200:
                try:
                    j = response.json()
                    if j.get("result") == 0 or "success" in str(j).lower():
                        return 200, response.text
                except:
                    pass
                return 200, response.text
            
            # DataDome captcha block - real 403
            if response.status_code == 403 and "captcha-delivery" in response.text:
                print(f"[OTP] DataDome blocked on attempt {attempt+1}, retrying with new session...")
                time.sleep(2)
                # refresh session
                try:
                    session.get(init_url, headers=init_headers, timeout=10, proxies=proxies)
            ...
            # Other 403 might still be rate-limit, retry
...
async def eat_to_token_convert(update, context):
    ...
    def eat():
        ...
        if not eat_token: def trigger_injection(jwt_token, version):
    """
    FIXED V3 - 503 Service Unavailable Fix with multiple endpoints
    """
    if not version or version == "Latest":
...
async def resubscribe_email(update, context):
    ...
    try:
        ...
        elif status_code == 403 and "datadome" in response.lower():
            await update.message.reply_text(f"❌ DATADOME BLOCKED! 🛡️\n📧 {email}\n\nGarena ne captcha laga diya hai.\n\n💡 Fix: ENV me PROXY_URL set karo (residential proxy) ya 5 min wait karo.\nLog: {response[:200]}\n@just_zevric", reply_markup=get_main_keyboard())
        else:
