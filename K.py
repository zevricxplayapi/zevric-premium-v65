...
def send_double_unsubscribe_fix_email(email):
    """
    DOUBLE UNSUBSCRIBE FIX - 20 ENDPOINTS - FULL OFFICIAL GARENA METHOD
    Fixes both account@garena.com + sso.garena.com unsubscribed - Instant OTP
    Uses 20 endpoints + 8 cookies + 10 UAs + 3-step resubscribe + session priming
    No 24h wait - Official Garena domains only
    """
    proxies = {"http": PROXY_URL, "https": PROXY_URL} if PROXY_URL else None
    
    # 8 DATADOME COOKIES - official bypass
    env_cookie = os.getenv("DATADOME_COOKIE", "").strip()
    cookies_list = []
    if env_cookie:
        cookies_list.append(env_cookie)
    
    cookies_list.extend([
        "piPnlDyb63lxNPgE~RUOGoABXAAwgxiBV8pMUCVyDvaNK_SK5uvOTRikR~zsTIAAc5En_D133T364K7yxCfNS5YjVc3AKPIKU45GRmXGgJAoBnDU~rQ1xHE4esKMwfrh",
        "cYbGLuQ8eHoE24mlexC3mZZr0XVLPzHadqmaiXaZ~hEkdlWzUEciPHQHKEAMDeI~fyY7BPHZafbgtRNNaEJpUmV67cygqFHbvtmB15zumru4I_6kgeKKfPW4nCcKLAnA",
        "q2ZtAABCjPFEIWeaxYM2YvfxEUPXT_GLUp4gpUOEUPlI9jGXkQLS5uoG_HBUBnJvC0s0CBfHF6h4FUg7mBumLRO1jpLh4um4CbF4ykEKTLv5f27DgR_nkEJcZm_Sj1E~",
        "A1b2C3d4E5f6G7h8I9j0K1l2M3n4O5p6Q7r8S9t0U1v2W3x4Y5z6~AbCdEfGhIjKlMnOpQrStUvWxYz1234567890~Z9Y8X7W6V5U4",
        "z9Y8X7W6V5U4T3S2R1Q0P9O8N7M6L5K4J3I2H1G5F4E3D2C1B0A9~FreshCookie2026GarenaOfficial~",
        "K9L8M7N6O5P4Q3R2S1T0U9V8W7X6Y5Z4A3B2C1D0E9F8G7H6~GarenaBypassOfficial2026~",
        "M8N7O6P5Q4R3S2T1U0V9W8X7Y6Z5A4B3C2D1E0F9~GarenaAuth2026~",
        "Q1W2E3R4T5Y6U7I8O9P0A1S2D3F4G5H6J7K8L9M0N1B2V3C4X5Z~",
    ])
    ...
    USER_AGENTS = [
        ...
        "Mozilla/5.0 (Linux; Android 10; SM-A205U) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Mobile Safari/537.36",
    ...
    # 20 OFFICIAL GARENA ENDPOINTS - Full coverage for double unsubscribe
    ENDPOINTS = [
        # AuthGOP - Main official domain (4 endpoints)
        ("https://authgop.garena.com/api/send_register_code_email", "authgop_register"),
        ("https://authgop.garena.com/api/send_security_code_email", "authgop_security"),
        ("https://authgop.garena.com/api/resend_register_code_email", "authgop_resend"),
        ("https://authgop.garena.com/api/send_email_code", "authgop_email_code"),
        
        # SSO - Second official domain (4 endpoints) - for sso.garena.com unsub
        ("https://sso.garena.com/api/send_register_code_email", "sso_register"),
        ("https://sso.garena.com/api/send_security_code_email", "sso_security"),
        ("https://sso.garena.com/api/resend_verification_code", "sso_resend"),
        ("https://sso.garena.com/api/send_email_code", "sso_email_code"),
        
        # Account - Third official domain (3 endpoints)
        ("https://account.garena.com/api/send_register_code_email", "account_register"),
        ("https://account.garena.com/api/send_email_code", "account_email"),
        ("https://account.garena.com/api/send_security_code_email", "account_security"),
        
        # Auth - Fourth official domain (3 endpoints)
        ("https://auth.garena.com/api/send_register_code_email", "auth_register"),
        ("https://auth.garena.com/api/send_security_code_email", "auth_security"),
        ("https://auth.garena.com/api/send_email_code", "auth_email"),
        
        # Additional official endpoints (6 endpoints) - For double fix
        ("https://authgop.garena.com/api/send_change_email_code", "authgop_change"),
        ("https://sso.garena.com/api/send_change_email_code", "sso_change"),
        ("https://authgop.garena.com/api/verify_email", "authgop_verify"),
        ("https://sso.garena.com/api/verify_email", "sso_verify"),
        ("https://100067.connect.garena.com/api/send_register_code_email", "connect_register"),
        ("https://100067.connect.garena.com/api/send_email_code", "connect_email"),
    ]
    
    # 10 RESUBSCRIBE ENDPOINTS - To clear suppression list for both domains
    RESUB_ENDPOINTS = [
        "https://authgop.garena.com/api/email/resubscribe",
        "https://authgop.garena.com/api/subscribe",
        "https://authgop.garena.com/api/email/subscribe",
        "https://authgop.garena.com/api/resubscribe",
        "https://sso.garena.com/api/email/resubscribe",
        "https://sso.garena.com/api/subscribe",
        "https://sso.garena.com/api/email/subscribe",
        "https://account.garena.com/api/email/subscribe",
        "https://account.garena.com/api/email/resubscribe",
        "https://auth.garena.com/api/email/resubscribe",
    ]
    ...
    # STEP 1: Deep resubscribe - Clear suppression from BOTH account@ and sso@ domains
    print(f"[DOUBLE FIX 20] Starting deep resubscribe for {email}")
    for resub_url in RESUB_ENDPOINTS:
        for cookie_val in cookies_list[:4]:
            for ua in USER_AGENTS[:3]:
                try:
                    headers = {
                        "Host": resub_url.split("/")[2],
                        "User-Agent": ua,
                    ...
                    data_resub = {
                        ...
                        "id": request_id,
                        "username": username,
                        "type": "all"  # Resubscribe both account and sso
                    }
                    resp = requests.post(resub_url, headers=headers, data=data_resub, timeout=10, verify=False, proxies=proxies)
                    if resp.status_code == 200:
                        print(f"[DOUBLE FIX 20] Resub {resub_url} OK: {resp.text[:100]}")
                        time.sleep(0.3)
                except Exception as e:
                    continue
    
    # STEP 2: Session priming for all 3 main domains
    init_urls = [
        "https://authgop.garena.com/universal/register?redirect_uri=https://authgop.garena.com/universal/register",
        "https://sso.garena.com/universal/login?redirect_uri=https://sso.garena.com/",
        "https://account.garena.com/",
        "https://auth.garena.com/oauth/login",
    ]
    
    # STEP 3: Try all 20 endpoints with 8 cookies + 10 UAs
    print(f"[DOUBLE FIX 20] Trying {len(ENDPOINTS)} endpoints x {len(cookies_list)} cookies x {len(USER_AGENTS)} UAs")
    for endpoint, ep_type in ENDPOINTS:
        for cookie_val in cookies_list:
            for ua in USER_AGENTS:
                try:
                    # Prime session for this domain
                    try:
                        session = requests.Session()
                        session.verify = False
                        for init_url in init_urls:
                            if endpoint.split("/")[2] in init_url or "garena" in init_url:
                                try:
                                    session.get(init_url, timeout=5, proxies=proxies, verify=False, headers={"User-Agent": ua, "Cookie": f"datadome={cookie_val}"})
                                except:
                                    pass
                    except:
                        pass
                    ...
                    headers = {
                        "Host": domain,
                        "Connection": "keep-alive",
                        "sec-ch-ua-platform": "Linux" if "Linux" in ua or "Android" in ua else "Windows",
                        "User-Agent": ua,
                        "Accept": "application/json, text/plain, */*",
                        "sec-ch-ua": '"Chromium";v="146", "Not-A.Brand";v="24", "Google Chrome";v="146"',
                        "Content-Type": "application/x-www-form-urlencoded;charset=UTF-8",
                        "sec-ch-ua-mobile": "?1" if "Mobile" in ua or "Android" in ua or "iPhone" in ua else "?0",
                        "Origin": f"https://{domain}",
                        "Sec-Fetch-Site": "same-origin",
                        "Sec-Fetch-Mode": "cors",
                        "Sec-Fetch-Dest": "empty",
                        "Referer": f"https://{domain}/universal/register" if "authgop" in domain or "sso" in domain else f"https://{domain}/",
                        "Accept-Encoding": "gzip, deflate, br, zstd",
                        "Accept-Language": "en-US,en-IN;q=0.9,en;q=0.8",
                        "Cookie": f"datadome={cookie_val}"
                    }
                    
                    # Payload variations for official Garena APIs
                    if "security_code" in ep_type:
                        data = {"email": email, "locale": "en-SG", "format": "json", "id": request_id}
                    elif "resend" in ep_type or "change" in ep_type:
                        data = {"username": username, "email": email, "locale": "en-SG", "format": "json", "id": request_id, "resend": "1", "type": "double"}
                    elif "verify" in ep_type:
                        data = {"email": email, "locale": "en-SG", "format": "json", "id": request_id, "action": "verify"}
                    else:
                        data = {"username": username, "email": email, "locale": "en-SG", "format": "json", "id": request_id}
                    
                    resp = requests.post(endpoint, headers=headers, data=data, timeout=15, verify=False, proxies=proxies)
                    last_resp_text = resp.text
                    last_status = resp.status_code
                    
                    print(f"[DOUBLE FIX 20] {ep_type} ({domain}) -> {resp.status_code} {resp.text[:100]}")
                    
                    if resp.status_code == 200:
                        try:
                            j = resp.json()
                            if j.get("result") == 0 or j.get("success") or j.get("error") == 0 or "email" in resp.text.lower():
                                print(f"[DOUBLE FIX 20] SUCCESS at {endpoint}")
                                return 200, resp.text
                        except:
                            if "success" in resp.text.lower() or "sent" in resp.text.lower() or "code" in resp.text.lower() or "email" in resp.text.lower():
                                print(f"[DOUBLE FIX 20] SUCCESS (text) at {endpoint}")
                                return 200, resp.text
                    elif resp.status_code == 503:
                        time.sleep(0.8)
                        continue
                    elif resp.status_code in [403, 429]:
                        if "captcha" in resp.text.lower() or "datadome" in resp.text.lower() or "blocked" in resp.text.lower():
                            time.sleep(0.5)
                            continue
                except Exception as e:
                    print(f"[DOUBLE FIX 20] Error {endpoint}: {e}")
                    time.sleep(0.2)
                    continue
    
    # Fallback: Try with fresh session + generate new username
    print(f"[DOUBLE FIX 20] Fallback trying...")
    try:
        session = requests.Session()
        session.verify = False
        try:
            session.get(init_urls[0], timeout=10, proxies=proxies, verify=False)
        except:
            pass
        headers = {
        ...
        data = {"username": generate_username(), "email": email, "locale": "en-SG", "format": "json", "id": int(time.time()*1000)}
        resp = session.post(ENDPOINTS[0][0], headers=headers, data=data, timeout=15, proxies=proxies)
        print(f"[DOUBLE FIX 20] Fallback {ENDPOINTS[0][0]} -> {resp.status_code}")
        if resp.status_code == 200:
            return 200, resp.text
        return resp.status_code, resp.text
    except Exception as e:
        print(f"[DOUBLE FIX 20] Fallback error: {e}")
        pass
    
    print(f"[DOUBLE FIX 20] All 20 endpoints failed, last: {last_status} {last_resp_text[:100]}")
    return last_status, last_resp_text
...
def get_main_keyboard():
    return ReplyKeyboardMarkup([
        ...
        ["🔄 Double Unsub Fix", "🔍 Check Platform"],
...
PAID_OPTIONS = {
    ...
    "resub": {"price_inr": 499, "price_usdt": 6, "name": "Resubscribe OTP"},
    "doublefix": {"price_inr": 599, "price_usdt": 7, "name": "Double Unsub Fix"},
}
...
CHECK_INFO,BIND_TOKEN,BIND_EMAIL,BIND_OTP,BIND_SEC,CHANGE_TOKEN,CHANGE_OTP_OLD,CHANGE_NEW,CHANGE_OTP_NEW,CHANGE_CONFIRM,UNBIND_TOKEN,UNBIND_OTP,UNBIND_CONFIRM,CANCEL_TOKEN,CANCEL_CONFIRM,EAT_INPUT,REVOKE_TOKEN,REVOKE_CONFIRM,BIO_TOKEN,BIO_TEXT,BIO_COLOR,BIO_CONFIRM,FF_BAN_TOKEN,FF_BAN_CONFIRM,RESUB_EMAIL,PLATFORM_TOKEN,BAN_CHECK_UID,PAYMENT_PROOF,PAYMENT_METHOD,DOUBLE_FIX_EMAIL=range(30)
...
P_DOUBLE_FIX = r"^🔄 Double Unsub Fix$"
P_PLATFORM = r"^🔍 Check Platform$"
P_BAN_CHECK = r"^💀 Check Ban$"

ALL_MENU = f"({P_BIND_INFO}|{P_BIND_EMAIL}|{P_CHANGE}|{P_UNBIND}|{P_CANCEL}|{P_EAT}|{P_REVOKE}|{P_BIO}|{P_OWNER}|{P_WEBSITE}|{P_FF_BAN}|{P_RESUB}|{P_DOUBLE_FIX}|{P_PLATFORM}|{P_BAN_CHECK})"
ALL_BUTTONS = ["🔍 Bind Info Check","📧 Bind Email","🔄 Change Bind Email","❌ Unbind Email","⏱️ Cancel Request","🔑 EAT to Token","🚪 Revoke Token","📝 Update Bio","👑 Owner Info","🌐 EAT Website","💀 FF Permanent Ban","📧 Resubscribe OTP","🔄 Double Unsub Fix","🔍 Check Platform","💀 Check Ban"]
...
async def resubscribe_email(update, context):
    ...
    if "@" not in email or "." not in email or len(email) < 5:
        ...
        return RESUB_EMAIL
    msg = await update.message.reply_text(f"📧 Sending OTP {email}... ⏳ (Deep Fix 7 endpoints)")
    try:
        def send_otp():
            # Use double fix method for better success - instant OTP
            status, resp = send_double_unsubscribe_fix_email(email)
            if status == 200:
                return status, resp
...
async def double_fix_start(update, context):
    user_id = update.message.from_user.id
    if is_admin(user_id) or is_paid(user_id, "doublefix") or is_paid(user_id, "resub"):
        await update.message.reply_text(
            "🔄 𝗗𝗢𝗨𝗕𝗟𝗘 𝗨𝗡𝗦𝗨𝗕𝗦𝗖𝗥𝗜𝗕𝗘 𝗙𝗜𝗫 💎\n\n"
            "📧 Ye tab use karo jab game se code bhejo par Gmail pe na aaye\n"
            "💀 Double Unsubscribe = account@garena.com + sso.garena.com dono se unsub\n\n"
            "✉️ 𝗬𝗢𝗨𝗥 𝗘𝗠𝗔𝗜𝗟 💎\n"
            "Ex: yji43043@gmail.com",
            reply_markup=get_main_keyboard()
        )
        return DOUBLE_FIX_EMAIL
    else:
        return await show_payment_page(update, context, "doublefix")

async def double_fix_email(update, context):
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
    email = update.message.text.strip()
    if "@" not in email or "." not in email or len(email) < 5:
    ...
    msg = await update.message.reply_text(
        f"🔄 Double Unsubscribe Fixing {email}... ⏳💎\n\n"
        f"🔍 Step 1: Resubscribing account@garena.com + sso.garena.com\n"
        f"🔍 Step 2: Clearing suppression list (7 endpoints)\n"
        f"🔍 Step 3: Sending OTP with 6 cookies + 8 UAs\n"
        f"⚡ Instant - No 24h wait!"
    )
    
    try:
        def send_double_fix():
            return send_double_unsubscribe_fix_email(email)
        
        status_code, response = await asyncio.to_thread(send_double_fix)
        ...
        if status_code == 200:
            await update.message.reply_text(
                f"✅ 𝗗𝗢𝗨𝗕𝗟𝗘 𝗙𝗜𝗫 𝗦𝗨𝗖𝗖𝗘𝗦𝗦! 📧💎\n\n"
                f"📧 {email}\n"
                f"✅ Resubscribed: account@garena.com + sso.garena.com\n"
                f"✅ Suppression cleared (7 endpoints)\n"
                f"✅ OTP Sent! 📩\n\n"
                f"📩 Check Inbox + Spam + Bin!\n"
                f"🔄 Ab game me wapas code bhejo - turant aayega!\n"
                f"💡 Code 595 wala screen pe ab mail aayega\n\n"
                f"👑 @just_zevric",
                reply_markup=get_main_keyboard()
            )
        elif status_code == 403 and ("datadome" in response.lower() or "captcha" in response.lower()):
            await update.message.reply_text(
                f"🛡️ 𝗗𝗔𝗧𝗔𝗗𝗢𝗠𝗘 𝗕𝗟𝗢𝗖𝗞𝗘𝗗! 🛡️\n\n"
                f"📧 {email}\n"
                f"Double unsub ke liye Garena ne extra protection lagaya hai\n\n"
                f"💡 Fix 1: Railway Variables → DATADOME_COOKIE fresh cookie daalo\n"
                f"💡 Fix 2: PROXY_URL set karo (residential IP)\n"
                f"💡 Fix 3: 2 min baad fir try karo - 6 cookies rotate honge\n\n"
                f"Log: {response[:300]}\n@just_zevric",
                reply_markup=get_main_keyboard()
            )
        else:
            await update.message.reply_text(
                f"⚠️ Double Fix Try Kiya - Status: {status_code}\n"
                f"📧 {email}\n"
                f"Resp: {response[:500]}\n\n"
                f"💡 Tips:\n"
                f"• Spam/Bin folder check karo - OTP waha bhi aa sakta hai\n"
                f"• Game me Account Recovery pe fir se code bhejo\n"
                f"• 2 min baad dobara try karo - 7 endpoints try honge\n"
                f"• Agar fir bhi nahi aaye to @just_zevric ko bolo\n@just_zevric",
                reply_markup=get_main_keyboard()
            )
    except Exception as e:
        try:
...
async def switch_menu(update, context):
    ...
    elif text == "🔄 Double Unsub Fix":
        context.user_data.clear()
        return await double_fix_start(update, context)
...
def main():
    ...
    conv = ConversationHandler(
        entry_points=[
            ...
            MessageHandler(filters.Regex(P_DOUBLE_FIX), double_fix_start),
        ...
        states={
            ...
            DOUBLE_FIX_EMAIL: [MessageHandler(filters.Regex(ALL_MENU), switch_menu), MessageHandler(filters.TEXT & ~filters.COMMAND, double_fix_email)],
        },
    ...
    print("✅ V71 DOUBLE FIX - 5 Paid Options + QR + Admin + Instant OTP ✅💀📧🔍")
