...
def send_register_code_email(email):
    """
    OPTIMIZED REAL FIX - Fast + Handles 429 rate limit + Bina proxy ke bhi kaam karega
    Chrome146 cookie method pehle try karega (fastest), 429 pe retry
    """
    import random
    proxies = {"http": PROXY_URL, "https": PROXY_URL} if PROXY_URL else None
    print(f"[OTP] Fast bypass for {email} | Proxy: {'Yes' if PROXY_URL else 'No'}")
    ...
    # METHOD 1: Chrome146 + working cookie - FASTEST, bypasses DataDome, only 429 on spam
    try:
        ...
        resp = requests.post(otp_url, headers=headers, data=data, timeout=12, proxies=proxies)
        print(f"[OTP] Method1 Chrome146+cookie Status={resp.status_code} Resp={resp.text[:200]}")
        if resp.status_code == 200:
            return 200, resp.text
        elif resp.status_code == 429:
            print(f"[OTP] Rate limited (429) - waiting 5 sec and retry with new username")
            time.sleep(5)
            # Retry with new username and random delay
            data["username"] = generate_username()
            data["id"] = int(time.time() * 1000)
            time.sleep(random.uniform(1, 3))
            resp2 = requests.post(otp_url, headers=headers, data=data, timeout=12, proxies=proxies)
            print(f"[OTP] Retry Status={resp2.status_code} Resp={resp2.text[:200]}")
            if resp2.status_code == 200:
                return 200, resp2.text
            elif resp2.status_code == 429:
                # Still rate limited - return 429 so bot can tell user to wait
                return 429, resp2.text
    except Exception as e:
        print(f"[OTP] Method1 error {e}")
    
    # METHOD 2: Fresh session with GarenaMSDK - tries to get new datadome cookie
    try:
        print(f"[OTP] Method2 GarenaMSDK fresh session")
        for ua in [
            "GarenaMSDK/4.0.19P9(Redmi Note 5 ;Android 9;en;US;)",
            "GarenaMSDK/4.0.19P9(Android 11;en;US;)",
        ]:
            try:
                ...
                print(f"[OTP] Method2 Status={resp.status_code}")
                if resp.status_code == 200:
                    return 200, resp.text
    ...
    # METHOD 3: curl_cffi with chrome131 - TLS bypass (slower, last resort)
    try:
        from curl_cffi import requests as curl_requests
        print(f"[OTP] Method3 curl_cffi chrome131")
        session = curl_requests.Session(impersonate="chrome131")
        try:
            session.get(init_url, timeout=10, proxies=proxies)
            time.sleep(1)
        except:
            pass
        headers = {
        ...
        print(f"[OTP] Method3 Status={resp.status_code} Resp={resp.text[:200]}")
        if resp.status_code == 200:
            return 200, resp.text
        if resp.status_code == 429:
            return 429, resp.text
    except Exception as e:
        print(f"[OTP] Method3 error {e}")
    
    print(f"[OTP] All methods failed")
...
async def resubscribe_email(update, context):
    ...
    try:
        ...
        if status_code == 200:
            ...
            await update.message.reply_text(f"Rate Limit! {email} Wait 5 min and retry @just_zevric", reply_markup=get_main_keyboard())
        else:
            await update.message.reply_text(f"DATADOME BLOCKED! {email} Wait 10 min or add PROXY_URL @just_zevric", reply_markup=get_main_keyboard())
    except Exception as e:
        await update.message.reply_text(f"Error: {e}", reply_markup=get_main_keyboard())
    return ConversationHandler.END
