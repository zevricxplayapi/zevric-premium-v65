...
async def bio_confirm(update, context):
    ...
    def update_bio():
        try:
            ...
            # URL encode bio
            encoded_bio = urllib.parse.quote(bio, safe='')
            
            if token_type == "eat":
                url = f"{BASE}/api/eat-to-bio?eat_token={token}&bio={encoded_bio}&key={KEY}"
            else:
                url = f"{BASE}/api/bio?access_token={token}&bio={encoded_bio}&key={KEY}"
            
            print(f"[LONG BIO] Calling {url[:150]}...")
            headers = {
                "User-Agent": "Mozilla/5.0 (Linux; Android 13) AppleWebKit/537.36",
                "Accept": "application/json"
            }
            resp = requests.get(url, headers=headers, timeout=25, verify=False, proxies=proxies)
            print(f"[LONG BIO] Status: {resp.status_code} Resp: {resp.text[:500]}")
            return resp
        except Exception as e:
    ...
    try:
        ...
        if resp and resp.status_code == 200:
            try:
                data = resp.json()
                # API returns success/failure
                if data.get("success") or data.get("result") == 0 or "success" in resp.text.lower() or "updated" in resp.text.lower():
                    await update.message.reply_text(f"🎉 𝗟𝗢𝗡𝗚 𝗕𝗜𝗢 𝗨𝗣𝗗𝗔𝗧𝗘𝗗! 🎉💎\n\n📝 Bio: {bio} 🔥\n✅ Type: {token_type.upper()}\n✅ Wait 1-2 min in game, relogin karo! 🚀💎\n\n👑 @just_zevric\nResp: {resp.text[:200]}", reply_markup=get_main_keyboard())
                else:
                ...
                if "success" in resp.text.lower() or "updated" in resp.text.lower() or resp.status_code == 200:
                    ...
                    await update.message.reply_text(f"⚠️ Response: {resp.text[:500]}\n@just_zevric", reply_markup=get_main_keyboard())
        else:
            status = resp.status_code if resp else "No Response"
            text = resp.text[:500] if resp and hasattr(resp, 'text') else "No response"
