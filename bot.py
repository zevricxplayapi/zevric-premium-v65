...
# ========== OTP LIMIT TRACKER - 10 OTP VALID THEN LIMIT ==========
OTP_LIMIT_FILE = "/tmp/otp_limit.json"
OTP_LIMIT_MAX = 10
OTP_LIMIT_WINDOW = 3600  # 1 hour

def load_otp_tracker():
    try:
        if os.path.exists(OTP_LIMIT_FILE):
            with open(OTP_LIMIT_FILE, 'r') as f:
                return json.load(f)
    except:
        pass
    return {}

def save_otp_tracker(data):
    try:
        with open(OTP_LIMIT_FILE, 'w') as f:
            json.dump(data, f)
    except:
        pass

def check_otp_limit(email):
    tracker = load_otp_tracker()
    email_key = email.lower().strip()
    now = time.time()
    if email_key not in tracker:
        return True, 0, 0
    entry = tracker[email_key]
    count = entry.get("count", 0)
    first_time = entry.get("first_time", now)
    # If window expired, reset
    if now - first_time > OTP_LIMIT_WINDOW:
        return True, 0, 0
    if count >= OTP_LIMIT_MAX:
        remaining = int(OTP_LIMIT_WINDOW - (now - first_time))
        return False, count, remaining
    return True, count, 0

def increment_otp_limit(email):
    tracker = load_otp_tracker()
    email_key = email.lower().strip()
    now = time.time()
    if email_key not in tracker:
        tracker[email_key] = {"count": 1, "first_time": now, "last_time": now}
    else:
        entry = tracker[email_key]
        first_time = entry.get("first_time", now)
        if now - first_time > OTP_LIMIT_WINDOW:
            ...
            tracker[email_key]["count"] = entry.get("count", 0) + 1
            tracker[email_key]["last_time"] = now
    save_otp_tracker(tracker)
    return tracker[email_key]["count"]
...
def fetch_majorlogin_jwt(tok):
    ...
    if tok.startswith("ey") and "." in tok and len(tok) > 100:
        return tok, None
    oId = None
    # Method 1: oauth inspect - no proxy
    try:
    ...
    for url in urls:
        for p_type in platforms:
            try:
                ...
                continue
    return None, "MajorLogin failed - Token expired or Railway DNS issue. Same as Subscribe_4.py without proxy"
...
def trigger_injection(jwt_token, version):
    ...
    try:
        ...
        print(f"[FFBAN] Injection error: {e}")
        class FakeResp:
            status_code = 0
            text = str(e)
        return FakeResp()
...
async def resubscribe_email(update, context):
    ...
    if "@" not in email or "." not in email or len(email) < 5:
        ...
        return RESUB_EMAIL
    # Check limit - 10 OTP valid then limit
    allowed, count, remaining = check_otp_limit(email)
    print(f"[OTP LIMIT] {email} count {count} allowed {allowed} remaining {remaining}s")
    if not allowed:
        mins = remaining // 60
        secs = remaining % 60
        nl = chr(10)
        msg = "🚫 𝗟𝗶𝗺𝗶𝘁 𝗥𝗲𝗮𝗰𝗵𝗲𝗱! ⛔" + nl + "📧 " + email + nl + f"✅ 10 OTP Already Sent!" + nl + f"⏰ Try After {mins}m {secs}s" + nl + "💡 Use Different Email @just_zevric"
        await update.message.reply_text(msg, reply_markup=get_main_keyboard())
        return ConversationHandler.END
        ...
        if response.status_code == 200:
            try:
                ...
                if ban_info and ban_info.get('is_banned'):
                    ...
                    txt = f"CROWNX BAN CHECKER Account ID: {account_id} Nickname: {nickname} Region: {region} Level: {level} Ban Status: PERMANENTLY BANNED Start Time: {ban_start} Reason: {ban_reason} @just_zevric"
                else:
                    txt = f"CROWNX BAN CHECKER Account ID: {account_id} Nickname: {nickname} Region: {region} Level: {level} Ban Status: ACCOUNT IS CLEAN (Not Banned) @just_zevric"
                await update.message.reply_text(txt, reply_markup=get_main_keyboard())
            except Exception as e:
                await update.message.reply_text(f"Ban Info for {uid}: {response.text[:400]} @just_zevric", reply_markup=get_main_keyboard())
        else:
            if response.status_code == 410:
                await update.message.reply_text(f"Ban Check API down (410 Gone). CrownX API removed. UID: {uid} Try again later or contact @just_zevric for new API @just_zevric", reply_markup=get_main_keyboard())
            elif response.status_code == 404:
                await update.message.reply_text(f"Ban Check API not found (404). UID: {uid} API endpoint changed. Contact @just_zevric @just_zevric", reply_markup=get_main_keyboard())
            elif response.status_code == 500:
                await update.message.reply_text(f"Ban Check API error (500). UID: {uid} Server error. Try again later @just_zevric", reply_markup=get_main_keyboard())
            else:
                await update.message.reply_text(f"Request Failed! UID: {uid} Status: {response.status_code} Response: {response.text[:200]} Try again later @just_zevric", reply_markup=get_main_keyboard())
