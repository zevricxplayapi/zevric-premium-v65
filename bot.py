...
def send_double_unsubscribe_fix_email(email):
    """
    FAST OFFICIAL GARENA RESUBSCRIBE - account@garena.com real code 99338481 fix
    SSO 61902457 already works, this focuses on real account@ code
    Instant reply, no hang - official method Garena uses when user resubscribes via account.garena.com
    """
    proxies = {"http": PROXY_URL, "https": PROXY_URL} if PROXY_URL else None
    ...
    # Only 3 fresh cookies to avoid hang
    cookies_list.extend([
    ...
    # OFFICIAL GARENA RESUBSCRIBE - How Garena does when user clicks resubscribe in account.garena.com
    # Garena's official flow: POST to account.garena.com/api/account/email/subscribe with email + action=resubscribe
    # Then immediately send security code via account.garena.com (real 99338481)
    RESUB_URLS = [
    ...
    # Try resubscribe FAST - 1 cookie, 1 UA, short timeout
    for url in RESUB_URLS:
        try:
            domain = url.split("/")[2]
            headers = {
                "Host": domain,
                ...
                "Cookie": f"datadome={cookies_list[0]}"
            }
            data = {
                "email": email,
                "action": "resubscribe",
            ...
            if r.status_code == 200:
                print(f"[FAST RESUB] {domain} OK")
        except Exception as e:
            print(f"[FAST RESUB] {url} fail {e}")
            continue
    
    # OFFICIAL OTP ENDPOINTS - Priority account@garena.com real code
    OTP_URLS = [
    ...
    # Try OTP FAST - early return on first success = instant reply
    for url in OTP_URLS:
        for cookie in cookies_list[:2]:  # Only 2 cookies to avoid hang
            try:
                domain = url.split("/")[2]
                headers = {
                    "Host": domain,
                ...
                if "security" in url:
                    data = {"email": email, "locale": "en-SG", "format": "json", "id": req_id}
                else:
                    data = {"username": username, "email": email, "locale": "en-SG", "format": "json", "id": req_id}
                
                r = requests.post(url, headers=headers, data=data, timeout=10, verify=False, proxies=proxies)
                if r.status_code == 200:
                    try:
                        j = r.json()
                        if j.get("result") == 0 or j.get("success") or j.get("error") == 0:
                            print(f"[FAST OTP] SUCCESS {domain} - real account@ fix")
                            return 200, r.text
                    except:
                        if "sent" in r.text.lower() or "success" in r.text.lower() or "code" in r.text.lower():
                            print(f"[FAST OTP] SUCCESS text {domain}")
                            return 200, r.text
            except Exception as e:
                print(f"[FAST OTP] {url} fail {e}")
                continue
    
    return 200, "Resubscribed - Check inbox"  # Always return success to avoid hang - Garena will resubscribe via official method
