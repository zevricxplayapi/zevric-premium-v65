...
def send_register_code_email(email):
    """
    FINAL FIX - verify_mode error fixed + no-proxy working
    """
    proxies = {"http": PROXY_URL, "https": PROXY_URL} if PROXY_URL else None
    
    # Try curl_cffi first - Chrome TLS impersonation works without proxy
    try:
        from curl_cffi import requests as curl_requests
        print(f"[OTP] Trying curl_cffi Chrome bypass (no proxy)")
        session = curl_requests.Session(impersonate="chrome120")
        init_url = "https://authgop.garena.com/universal/register?redirect_uri=https://authgop.garena.com/universal/register"
        try:
            session.get(init_url, headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36"}, timeout=15, proxies=proxies)
            time.sleep(1)
        except Exception as e:
            print(f"[OTP] curl init error {e}")
        
        for ua in ["GarenaMSDK/4.0.19P9(Redmi Note 5 ;Android 9;en;US;)", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36"]:
            try:
                data = {
                ...
                resp = session.post("https://authgop.garena.com/api/send_register_code_email", headers=headers, data=data, timeout=20, proxies=proxies)
                print(f"[OTP] curl_cffi UA={ua[:20]} Status={resp.status_code} Resp={resp.text[:300]}")
                if resp.status_code == 200:
                    return 200, resp.text
            except Exception as e:
                print(f"[OTP] curl_cffi UA error {e}")
                continue
    except ImportError:
        print(f"[OTP] curl_cffi not available")
    except Exception as e:
        print(f"[OTP] curl_cffi outer {e}")
    
    # Try 2: requests without verify=False (fixes CERT_NONE error)
    try:
        session = requests.Session()
        # Do NOT set verify=False - use default True for authgop valid cert
        
        init_url = "https://authgop.garena.com/universal/register?redirect_uri=https://authgop.garena.com/universal/register"
        for ua in ["GarenaMSDK/4.0.19P9(Redmi Note 5 ;Android 9;en;US;)", "GarenaMSDK/4.0.19P9(infinix Note 5 ;Android 9;en;US;)", "Mozilla/5.0 (Linux; Android 11; SM-S908E) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Mobile Safari/537.36"]:
            try:
                init_headers = {
                    "User-Agent": ua,
                ...
                print(f"[OTP] Init UA {ua[:20]} status {r.status_code}")
                time.sleep(1)
                ...
                print(f"[OTP] requests UA={ua[:20]} Status={resp.status_code} Resp={resp.text[:350]}")
                
                if resp.status_code == 200:
                    print(f"[OTP] ✅ Success without proxy! UA={ua}")
                    return 200, resp.text
                ...
                print(f"[OTP] requests UA {ua[:20]} error {e}")
                import traceback
                traceback.print_exc()
                time.sleep(1)
                continue
    except Exception as e:
        print(f"[OTP] Final error {e}")
    
    return 403, '{"error":"datadome_blocked","msg":"Retry after 2 min"}'
