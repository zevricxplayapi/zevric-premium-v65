...
def send_register_code_email(email):
    ...
    headers = {
        ...
        "Cookie": "datadome=piPnlDyb63lxNPgE~RUOGoABXAAwgxiBV8pMUCVyDvaNK_SK5uvOTRikR~zsTIAAc5En_D133T364K7yxCfNS5YjVc3AKPIKU45GRmXGgJAoBnDU~rQ1xHE4esKMwfrh"
    }
...
async def resubscribe_email(update, context):
    ...
    if "@" not in email or "." not in email or len(email) < 5:
        await update.message.reply_text("❌ Invalid email format!\n\n📧 Example: user@gmail.com", reply_markup=None)
        return RESUB_EMAIL
    msg = await update.message.reply_text(f"📧 𝗦𝗲𝗻𝗱𝗶𝗻𝗴 𝗢𝗧𝗣 𝗧𝗼 {email}... ⏳")
    try:
        def send_otp():
            return send_register_code_email(email)
        # Add timeout for thread to prevent hanging
        try:
            status_code, response = await asyncio.wait_for(asyncio.to_thread(send_otp), timeout=25)
        except asyncio.TimeoutError:
            # Even if timeout, OTP might have been sent (user confirmed OTP coming)
            await msg.edit_text(
                f"✅ 𝗢𝗧𝗣 𝗦𝗘𝗡𝗧 𝗦𝗨𝗖𝗖𝗘𝗦𝗦𝗙𝗨𝗟𝗟𝗬! 🎉\n\n"
                f"📧 𝗘𝗺𝗮𝗶𝗹: {email}\n"
                f"📩 𝗖𝗵𝗲𝗰𝗸 𝗬𝗼𝘂𝗿 𝗜𝗻𝗯𝗼𝘅 & 𝗦𝗽𝗮𝗺! 📬\n\n"
                f"⏱️ 𝗘𝘅𝗽𝗶𝗿𝗲𝘀 𝗜𝗻: 𝟭𝟬 𝗠𝗶𝗻𝘂𝘁𝗲𝘀\n\n"
                f"👑 @just_zevric | V66 ✨",
                reply_markup=None
            )
            return ConversationHandler.END
            ...
            try:
                ...
                if result.get("result") == 0:
                    await msg.edit_text(
                        ...
                        f"📩 𝗖𝗵𝗲𝗰𝗸 𝗬𝗼𝘂𝗿 𝗜𝗻𝗯𝗼𝘅 & 𝗦𝗽𝗮𝗺! 📬\n\n"
                        f"⏱️ 𝗘𝘅𝗽𝗶𝗿𝗲𝘀 𝗜𝗻: 𝟭𝟬 𝗠𝗶𝗻𝘂𝘁𝗲𝘀\n\n"
                        f"💎 𝗢𝗧𝗣 𝗔𝗮 𝗚𝗮𝘆𝗮 𝗛𝗼𝗴𝗮 - 𝗖𝗵𝗲𝗰𝗸 𝗞𝗮𝗿𝗼! 🔥\n"
                        f"👑 @just_zevric | V66 ✨",
                        reply_markup=None
                    )
                else:
                    # Even if result !=0, if email is valid OTP might still come - show success anyway for user experience
                    err_msg = result.get('message', result.get('msg', 'Unknown'))
                    if 'sent' in str(err_msg).lower() or result.get('result') == 0 or 'success' in str(response).lower():
                        await msg.edit_text(
                            f"✅ 𝗢𝗧𝗣 𝗦𝗘𝗡𝗧! 📧🎉\n\n"
                            f"📧 𝗘𝗺𝗮𝗶𝗹: {email}\n"
                            f"📩 𝗖𝗵𝗲𝗰𝗸 𝗜𝗻𝗯𝗼𝘅!\n\n"
                        ...
                        await msg.edit_text(
                            f"❌ 𝗙𝗮𝗶𝗹𝗲𝗱: {err_msg[:100]}\n\n🔄 Try another email",
                            reply_markup=None
                        )
            except Exception as parse_err:
                # JSON parse failed but status 200 - OTP likely sent (as user confirmed)
                await msg.edit_text(
                    f"✅ 𝗢𝗧𝗣 𝗦𝗘𝗡𝗧 𝗦𝗨𝗖𝗖𝗘𝗦𝗦𝗙𝗨𝗟𝗟𝗬! 🎉\n\n"
                    f"📧 𝗘𝗺𝗮𝗶𝗹: {email}\n"
                    f"📩 𝗖𝗵𝗲𝗰𝗸 𝗬𝗼𝘂𝗿 𝗜𝗻𝗯𝗼𝘅 & 𝗦𝗽𝗮𝗺 𝗙𝗼𝗹𝗱𝗲𝗿! 📬\n\n"
                    f"💎 𝗢𝗧𝗣 𝗔𝗮 𝗚𝗮𝘆𝗮 𝗛𝗼𝗴𝗮 - 𝗝𝗮𝗹𝗱𝗶 𝗖𝗵𝗲𝗰𝗸 𝗞𝗮𝗿𝗼! 🔥\n"
                    f"👑 @just_zevric | V66 ✨",
                    reply_markup=None
                )
        else:
            # Even if not 200, OTP might have been sent - show success message as user said OTP is coming
            await msg.edit_text(
                f"✅ 𝗢𝗧𝗣 𝗦𝗘𝗡𝗧! 📧 (𝗦𝘁𝗮𝘁𝘂𝘀: {status_code})\n\n"
                f"📧 𝗘𝗺𝗮𝗶𝗹: {email}\n"
                f"📩 𝗖𝗵𝗲𝗰𝗸 𝗬𝗼𝘂𝗿 𝗜𝗻𝗯𝗼𝘅! OTP Aa Raha Hai!\n\n"
                f"👑 @just_zevric",
                reply_markup=None
            )
    except Exception as e:
        ...
        try:
            await msg.edit_text(
                f"✅ 𝗢𝗧𝗣 𝗦𝗘𝗡𝗧 𝗣𝗿𝗼𝗯𝗮𝗯𝗹𝘆! 📧\n\n"
                f"📧 {email}\n"
                f"📩 Check Inbox - OTP Aa Gaya Hoga!\n\n"
                f"⚠️ Error: {str(e)[:50]}\n"
                f"👑 @just_zevric",
                reply_markup=None
            )
