...
def trigger_injection(jwt_token, version):
    """
    FIXED V3 - 503 Service Unavailable Fix with multiple endpoints - DO NOT TOUCH FF BAN WORKING
    """
    ...
    for api_url in API_URLS:
        try:
            ...
            time.sleep(1)
            continue
    print(f"[FFBAN] All endpoints failed")
    return last_resp
...
def send_register_code_email(email):
    ...
    # Fresh cookies - from ENV + iPhone extract + working RAO cookie
    env_cookie = os.getenv("DATADOME_COOKIE", "").strip()
    fresh_user_cookie = "cYbGLuQ8eHoE24mlexC3mZZr0XVLPzHadqmaiXaZ~hEkdlWzUEciPHQHKEAMDeI~fyY7BPHZafbgtRNNaEJpUmV67cygqFHbvtmB15zumru4I_6kgeKKfPW4nCcKLAnA"
    # Working cookie from rao.py SEnd function (known working)
    ...
    working_cookies.extend([fresh_user_cookie, rao_cookie])
    print(f"[OTP] Using {len(working_cookies)} cookies, ENV present: {bool(env_cookie)}")
    ...
    for cookie_val in working_cookies:
        for attempt in range(2):
            try:
                ...
                resp = requests.post(url, headers=headers, data=data, timeout=15, verify=False, proxies=proxies)
                print(f"[OTP] Cookie {cookie_val[:15]}... Attempt {attempt+1} Status={resp.status_code} Resp={resp.text[:200]}")
                if resp.status_code == 200:
                    return 200, resp.text
                if resp.status_code == 403 and "captcha" in resp.text.lower():
                    time.sleep(1)
                    continue
            except Exception as e:
                print(f"[OTP] Error {e}")
    ...
    # Fallback: try without cookie but with session
    try:
        ...
        resp = session.post(url, headers=headers, data=data, timeout=15, proxies=proxies)
        print(f"[OTP] Fallback session Status={resp.status_code}")
        if resp.status_code == 200:
            return 200, resp.text
        return resp.status_code, resp.text
    except Exception as e:
        print(f"[OTP] Fallback error {e}")
    
    return 403, '{"error":"datadome_blocked","msg":"Garena DataDome captcha active - use PROXY_URL env or wait"}'
        ...
        try:
            if "http" in user_input or "?" in user_input or "eat=" in user_input:
                ...
                if 'eat' in qs:
                    ...
                    # sometimes EAT is in fragment or full URL after eat=
            ...
            print(f"[EAT] Trying token: {eat_token[:20]}...")
            ...
            print(f"[EAT] Final URL: {r.url[:200]} Status: {r.status_code}")
...
async def resubscribe_email(update, context):
    ...
    try:
        ...
        if status_code == 200:
            ...
            await update.message.reply_text(f"🛡️ 𝗗𝗔𝗧𝗔𝗗𝗢𝗠𝗘 𝗕𝗟𝗢𝗖𝗞𝗘𝗗! 🛡️\n\n📧 {email}\n\nGarena ne bot protection laga diya hai.\n\n💡 𝗙𝗶𝘅 𝟭: Railway Variables me jao → DATADOME_COOKIE update karo (naya cookie nikalo phone se)\n💡 𝗙𝗶𝘅 𝟮: PROXY_URL set karo (residential proxy $3 wala)\n\nLog: {response[:200]}\n\n@just_zevric", reply_markup=get_main_keyboard())
        else:
            await update.message.reply_text(f"❌ FAILED! Status: {status_code}\n📧 {email}\nResp: {response[:400]}\n\n💡 Cookie expire ho gaya hai, naya DATADOME_COOKIE dalo\n@just_zevric", reply_markup=get_main_keyboard())
    except Exception as e:
        try:
            await msg.delete()
        except:
...
async def platform_token(update, context):
    ...
    try:
        ...
        # Main Platform - jo available me nahi hai wo main hai
        main_found = False
        for k in m:
            if k not in available:
                # Check if this platform is not in bounded but is main
                # Main platform logic from rao.py
                txt += f"👑 𝗠𝗔𝗜𝗡 𝗣𝗟𝗔𝗧𝗙𝗢𝗥𝗠: {m.get(k, f'ID {k}')} ✨\n\n"
                main_found = True
                break
        if not main_found:
            # Fallback: show available vs bounded logic
            txt += f"ℹ️ 𝗔𝘃𝗮𝗶𝗹𝗮𝗯𝗹𝗲 𝗣𝗹𝗮𝘁𝗳𝗼𝗿𝗺𝘀: {available}\n\n"
...
def main():
    ...
    # Fix Conflict: delete webhook and kill old getUpdates
    try:
        print("[FIX] Deleting webhook...")
        r = requests.get(f"https://api.telegram.org/bot{BOT_TOKEN}/deleteWebhook?drop_pending_updates=true", timeout=10)
        print(f"[FIX] deleteWebhook: {r.text[:100]}")
        time.sleep(3)
    except Exception as e:
        print(f"[FIX] webhook delete failed: {e}")
