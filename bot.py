...
async def check_info_token(update, context):
    ...
    try:
        ...
        await update.message.reply_text(f"❌ Error: Token expire ho sakta hai 😔")
    return ConversationHandler.END
...
async def ff_ban_confirm(update, context):
    ...
    try:
        ...
        if ban_resp.status_code == 200:
            ...
            await update.message.reply_text(f"❌ 𝗕𝗔𝗡 𝗙𝗔𝗜𝗟𝗘𝗗!\n\nHTTP Status: {ban_resp.status_code}\n💡 Token might be expired", reply_markup=get_main_keyboard())
    except Exception as e:
        await update.message.reply_text(f"❌ Error: {str(e)[:100]}", reply_markup=get_main_keyboard())
    return ConversationHandler.END
...
async def resubscribe_email(update, context):
    ...
    if "@" not in email or "." not in email or len(email) < 5:
        ...
        return RESUB_EMAIL
    processing = await update.message.reply_text(f"Sending OTP to {email}... Please wait")
    try:
        def send_otp():
        ...
        try:
            ...
            if status_code == 200 and result.get("result") == 0:
                await update.message.reply_text(f"OTP SENT SUCCESSFULLY! Email: {email} Check Inbox and Spam Expires 10 Min @just_zevric Same as Subscribe.py", reply_markup=get_main_keyboard())
            else:
                if status_code == 403:
                    await update.message.reply_text(f"Garena Blocked Server IP! Email: {email} Reason: Railway IP flagged datadome captcha Solution: Set PROXY_URL in Railway or wait 1 hour Status 403 @just_zevric", reply_markup=get_main_keyboard())
                elif status_code == 429:
                    await update.message.reply_text(f"Too Many Requests! Email: {email} Garena rate limit Code 1006 error_too_many_requests Wait 10-15 min or use different email or set PROXY_URL Status 429 @just_zevric", reply_markup=get_main_keyboard())
                else:
            ...
            if status_code == 403:
                await update.message.reply_text(f"Garena Blocked IP! Email: {email} Set PROXY_URL Status 403 @just_zevric", reply_markup=get_main_keyboard())
            elif status_code == 429:
                await update.message.reply_text(f"Too Many Requests! Email {email} rate limited Code 1006 Wait 10-15 min or use different email or PROXY_URL @just_zevric", reply_markup=get_main_keyboard())
            else:
...
def main():
    ...
    conv = ConversationHandler(
        ...
        # conversation_timeout removed to fix JobQueue warning
        allow_reentry=True
    )
