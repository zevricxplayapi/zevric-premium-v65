...
async def bio_token(update, context):
    ...
    # Detect EAT vs Access by length and format
    # EAT tokens are usually 32-64 hex, Access tokens are longer (100+) or JWT (ey...)
    if token_type != "eat":
...
async def bio_text(update, context):
    ...
    # Website has maxlength=280, so we match that for reliability
    # Long bio >280 often fails with No Response
    if len(bio_raw) < 1 or len(bio_raw) > 500:
        await update.message.reply_text(f"❌ 1-500 chars! Now {len(bio_raw)}\n💡 Garena limit is 300, but Long Bio API supports up to 500 with exploit. Website limit is 280 for safety. Try 280 or less for best success", reply_markup=get_main_keyboard())
        return BIO_TEXT
    if len(bio_raw) > 280:
        await update.message.reply_text(f"⚠️ Bio {len(bio_raw)} chars hai (280 se zyada)\n📏 Long Bio try kar rahe hain, fail ho sakta hai Garena limit ki wajah se\n💡 280 tak best success rate hai (website bhi 280 limit rakhti hai)", reply_markup=get_main_keyboard())
...
async def bio_color(update, context):
    ...
    if not plain:
        # If plain missing but bio_final exists, use that
        if 'bio_final' in context.user_data:
            await update.message.reply_text(f"👁️ Preview: {context.user_data['bio_final'][:150]} 🔥\n\n✅ Update? 🚀💎", reply_markup=get_confirm_keyboard())
            return BIO_CONFIRM
...
async def bio_confirm(update, context):
    ...
    msg = await update.message.reply_text(f"⏳ Updating Bio... 🚀💎\n📝 {bio[:80]} ✨\n🔑 Type: {token_type}\n📏 Length: {len(bio)} chars\n🌐 API: ff-long-bio-update-tools", reply_markup=get_main_keyboard())
    ...
    def update_bio():
        try:
            ...
            # Website ka exact method - GET with encodeURIComponent (like index.html)
            # Website also has maxlength=280, so long bio >280 may fail
            headers = {
                "User-Agent": "Mozilla/5.0 (Linux; Android 13) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Mobile Safari/537.36",
                "Accept": "application/json, text/plain, */*",
                "Accept-Language": "en-US,en;q=0.9",
                "Referer": "https://ff-long-bio-update-tools.vercel.app/",
                "Origin": "https://ff-long-bio-update-tools.vercel.app",
            ...
            if token_type == "eat":
                ...
                url = f"{BASE}/api/bio?access_token={urllib.parse.quote(token, safe='')}&bio={urllib.parse.quote(bio, safe='')}&key={KEY}"
            
            print(f"[BIO] Calling {url[:200]}... | Len: {len(bio)}")
            
            # Try GET like website does
            try:
                resp = requests.get(url, headers=headers, timeout=30, verify=False, proxies=proxies)
                print(f"[BIO] GET Status: {resp.status_code} Resp: {resp.text[:800]}")
                if resp.status_code == 200:
                    return resp
                # If GET fails with 414 (URI Too Long) for long bio, try POST
                if resp.status_code == 414 or len(bio) > 280:
                    print(f"[BIO] GET 414 or long bio, trying POST...")
                    # Try POST with JSON
                    try:
                        post_url = f"{BASE}/api/eat-to-bio" if token_type == "eat" else f"{BASE}/api/bio"
                        payload = {"eat_token": token, "bio": bio, "key": KEY} if token_type == "eat" else {"access_token": token, "bio": bio, "key": KEY}
                        resp_post = requests.post(post_url, json=payload, headers={**headers, "Content-Type": "application/json"}, timeout=30, verify=False, proxies=proxies)
                        print(f"[BIO] POST Status: {resp_post.status_code} Resp: {resp_post.text[:800]}")
                        if resp_post.status_code == 200:
                            return resp_post
                    except Exception as e:
                        print(f"[BIO] POST Error: {e}")
                return resp
            except Exception as e:
                print(f"[BIO] GET Error: {e}")
                ...
                try:
                    ...
                    print(f"[BIO] POST Fallback Error: {e2}")
                    return None
    ...
    try:
        resp = await asyncio.to_thread(update_bio)
        try:
            await msg.delete()
        except:
            pass
        ...
        if resp and resp.status_code == 200:
            try:
                ...
                if data.get("status") == "success" or data.get("success") or data.get("result") == 0 or "success" in resp.text.lower() or "updated" in resp.text.lower():
                    nickname = data.get("account_nickname") or data.get("bio_update", {}).get("nickname") or "—"
                    uid = data.get("account_id") or data.get("bio_update", {}).get("uid") or "—"
                    await update.message.reply_text(f"🎉 𝗕𝗜𝗢 𝗨𝗣𝗗𝗔𝗧𝗘𝗗! 🎉💎\n\n👤 Nick: {nickname}\n🆔 UID: {uid}\n📝 Bio: {bio[:200]} 🔥\n📏 Length: {len(bio)} chars\n✅ Type: {token_type.upper()}\n✅ Wait 1-2 min, relogin karo! 🚀💎\n\n👑 @just_zevric", reply_markup=get_main_keyboard())
                else:
                    await update.message.reply_text(f"⚠️ API Response:\n{resp.text[:700]}\n\n💡 Token expire ya bio too long (280+ chars Garena limit). 280 chars tak try karo (website bhi 280 limit rakhti hai)\n@just_zevric", reply_markup=get_main_keyboard())
            except:
                if "success" in resp.text.lower() or "updated" in resp.text.lower():
                    await update.message.reply_text(f"🎉 𝗕𝗜𝗢 𝗨𝗣𝗗𝗔𝗧𝗘𝗗! 🎉💎\n📝 Bio: {bio[:200]} 🔥\n📏 {len(bio)} chars\n✅ Wait 2 min! 🚀💎\n👑 @just_zevric", reply_markup=get_main_keyboard())
                else:
                    await update.message.reply_text(f"⚠️ Response: {resp.text[:700]}\n@just_zevric", reply_markup=get_main_keyboard())
        else:
            status = resp.status_code if resp and hasattr(resp, 'status_code') else "No Response"
            text = resp.text[:700] if resp and hasattr(resp, 'text') else "No response - API down, token expired, ya bio too long"
            if "No Response" in str(status):
                await update.message.reply_text(f"❌ FAILED! Status: {status}\nResp: {text}\n\n💡 Fix:\n• Bio 280 chars se kam rakho (website limit 280 hai, Garena limit 300 hai)\n• Fresh token/EAT use karo\n• API down ho sakta hai, 5 min baad try karo\n• PROXY_URL set karo agar Railway IP block hai\n@just_zevric", reply_markup=get_main_keyboard())
            else:
                await update.message.reply_text(f"❌ FAILED! Status: {status}\nResp: {text}\n\n💡 Bio 280 tak rakho, fresh token try karo\n@just_zevric", reply_markup=get_main_keyboard())
    except Exception as e:
        try:
            await msg.delete()
        except:
