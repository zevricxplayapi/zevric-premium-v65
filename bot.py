...
def send_register_code_email(email):
    """
    STABLE RAILWAY FIX - No Playwright, 3 methods, no build crash
    """
    import random
    proxies = {"http": PROXY_URL, "https": PROXY_URL} if PROXY_URL else None
    print(f"[OTP] Stable bypass for {email} | Proxy: {'Yes' if PROXY_URL else 'No'}")
    ...
    # Check for fresh cookie from ENV
    env_cookie = os.getenv("DATADOME_COOKIE", "")
    if env_cookie:
        print(f"[OTP] Using ENV DATADOME_COOKIE")
        working_cookies = [env_cookie, "piPnlDyb63lxNPgE~RUOGoABXAAwgxiBV8pMUCVyDvaNK_SK5uvOTRikR~zsTIAAc5En_D133T364K7yxCfNS5YjVc3AKPIKU45GRmXGgJAoBnDU~rQ1xHE4esKMwfrh"]
    else:
        working_cookies = ["piPnlDyb63lxNPgE~RUOGoABXAAwgxiBV8pMUCVyDvaNK_SK5uvOTRikR~zsTIAAc5En_D133T364K7yxCfNS5YjVc3AKPIKU45GRmXGgJAoBnDU~rQ1xHE4esKMwfrh"]
    
    # METHOD 1: Chrome146 + datadome cookie - FASTEST
    ...
    # METHOD 2: GarenaMSDK
    try:
        print(f"[OTP] Method2 GarenaMSDK")
        for ua in ["GarenaMSDK/4.0.19P9(Redmi Note 5 ;Android 9;en;US;)", "GarenaMSDK/4.0.19P9(Android 11;en;US;)"]:
            try:
    ...
    try:
        ...
        print(f"[OTP] Method3 curl_cffi")
        session = curl_requests.Session(impersonate="chrome131")
        try:
            session.get(init_url, timeout=8, proxies=proxies)
            time.sleep(0.8)
        except:
            pass
        headers = {
        ...
        resp = session.post(otp_url, headers=headers, data=data, timeout=12, proxies=proxies)
        print(f"[OTP] Method3 Status={resp.status_code}")
        if resp.status_code == 200:
            return 200, resp.text
        if resp.status_code == 429:
            return 429, resp.text
    ...
    print(f"[OTP] All 3 methods failed")
...
async def resubscribe_email(update, context):
    ...
    try:
        ...
        try:
            ...
            pass
        if status_code == 200:
            await update.message.reply_text(f"OTP SENT! {email} @just_zevric", reply_markup=get_main_keyboard())
        elif status_code == 429:
            await update.message.reply_text(f"Rate Limit! {email} Wait 5 min @just_zevric", reply_markup=get_main_keyboard())
        else:
            await update.message.reply_text(f"BLOCKED! {email} Add PROXY_URL or DATADOME_COOKIE in ENV @just_zevric", reply_markup=get_main_keyboard())
    except Exception as e:
        await update.message.reply_text(f"Error: {e}", reply_markup=get_main_keyboard())
    return ConversationHandler.END
