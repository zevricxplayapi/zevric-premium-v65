    ...
    for attempt in range(3):
        try:
            ...
            if response.status_code == 403 and "captcha-delivery" in response.text:
                print(f"[OTP] DataDome blocked on attempt {attempt+1}, retrying...")
                time.sleep(2)
                try:
                    session.get(init_url, headers=init_headers, timeout=10, proxies=proxies)
                except:
...
async def ff_ban_token(update, context):
    ...
    try:
        ...
        tmp_data = decode_jwt(jwt_token)
        context.user_data['ff_ban_version'] = tmp_data.get('release_version', tmp_data.get('client_version', 'OB54'))
        user_data = tmp_data
