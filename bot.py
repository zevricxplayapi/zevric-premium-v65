...
def send_register_code_email(email):
    """
    REAL OTP FIX WITHOUT PROXY - Multi-method bypass like FF Ban
    Tries 5 different ways, same as FF Ban multi-endpoint fix
    """
    proxies = {"http": PROXY_URL, "https": PROXY_URL} if PROXY_URL else None
    print(f"[OTP] Starting multi-bypass for {email} | Proxy: {'Yes' if PROXY_URL else 'No'}")
    ...
    # Method 1: curl_cffi Chrome131 - Best TLS bypass
    try:
        from curl_cffi import requests as curl_requests
        for imp in ["chrome131", "chrome", "safari", "edge101"]:
            try:
                print(f"[OTP] Trying curl_cffi impersonate={imp}")
                ...
                try:
                    ...
                    print(f"[OTP] Warmup error {e}")
                
                # Step 2: Send OTP with same session
                headers = {
                ...
                resp = session.post(otp_url, headers=headers, data=data, timeout=20, proxies=proxies)
                print(f"[OTP] {imp} Status={resp.status_code} Resp={resp.text[:300]}")
                
                if resp.status_code == 200:
                    try:
                        j = resp.json()
                        if j.get("result") == 0 or "result" in resp.text:
                            print(f"[OTP] SUCCESS with {imp} - REAL 200")
                            return 200, resp.text
                    except:
                        return 200, resp.text
                
                # If 403 with captcha url, try visiting captcha then retry
                if resp.status_code == 403 and "captcha-delivery" in resp.text:
                    try:
                        j = resp.json()
                        cap_url = j.get("url", "")
                        if cap_url:
                            print(f"[OTP] Visiting captcha interstitial...")
                            session.get(cap_url, headers={"User-Agent": headers["User-Agent"]}, timeout=10, proxies=proxies)
                            time.sleep(2)
                            resp2 = session.post(otp_url, headers=headers, data=data, timeout=20, proxies=proxies)
                            print(f"[OTP] Retry after captcha Status={resp2.status_code}")
                ...
                print(f"[OTP] {imp} error {e}")
                continue
    except ImportError:
        print("[OTP] curl_cffi not available, skipping")
    except Exception as e:
        print(f"[OTP] curl_cffi outer error {e}")
    
    # Method 2: GarenaMSDK UA - Whitelisted SDK bypass
    try:
        print(f"[OTP] Trying GarenaMSDK whitelist bypass")
        for ua in [
            ...
            try:
                ...
                session.get(init_url, headers={"User-Agent": ua}, timeout=10, proxies=proxies)
                time.sleep(1)
                ...
                print(f"[OTP] GarenaMSDK UA={ua[:25]} Status={resp.status_code}")
                if resp.status_code == 200:
                    return 200, resp.text
            except Exception as e:
                print(f"[OTP] GarenaMSDK error {e}")
                continue
    except Exception as e:
        print(f"[OTP] GarenaMSDK outer {e}")
    
    # Method 3: Direct Chrome 146 with working datadome cookie (local working method)
    try:
        print(f"[OTP] Trying Chrome146 + datadome cookie method")
        ...
        headers = {
            ...
            "Origin": "https://authgop.garena.com",
            "Referer": init_url,
            "Cookie": f"_ga=GA1.1.868059803.1775926376; datadome={working_datadome}"
        }
        resp = requests.post(otp_url, headers=headers, data=data, timeout=15, proxies=proxies)
        print(f"[OTP] Chrome146+cookie Status={resp.status_code} Resp={resp.text[:300]}")
        if resp.status_code == 200:
            return 200, resp.text
    except Exception as e:
        print(f"[OTP] Chrome146 method error {e}")
    
    # All methods failed
    print(f"[OTP] All bypass methods failed, returning 403")
    return 403, '{"error":"datadome_blocked","msg":"All methods failed"}'
