...
async def ban_check_uid(update, context):
    ...
    try:
        def fetch_ban():
            ...
            proxies = {"http": PROXY_URL, "https": PROXY_URL} if PROXY_URL else None
            return requests.get(url, timeout=15, verify=False, proxies=proxies)
        ...
        if response.status_code == 200:
            ...
            if response.status_code == 410:
                await update.message.reply_text(f"⚠️ Ban Check API Removed (410){chr(10)}UID: {uid}{chr(10)}CrownX API is down{chr(10)}Contact @just_zevric for new API", reply_markup=get_main_keyboard())
            elif response.status_code == 404:
                await update.message.reply_text(f"⚠️ Ban Check API Not Found (404){chr(10)}UID: {uid}{chr(10)}Try later @just_zevric", reply_markup=get_main_keyboard())
            else:
                await update.message.reply_text(f"⚠️ Ban Check Failed ({response.status_code}){chr(10)}UID: {uid}{chr(10)}Try later @just_zevric", reply_markup=get_main_keyboard())
    except Exception as e:
        try:
            await processing.delete()
        except:
