...
def send_register_code_email(email):
    ...
    # Fresh cookies - from ENV + user iPhone extract 2:11 PM
    ...
    if env_cookie:
        working_cookies = [env_cookie, fresh_user_cookie]
        print(f"[OTP] Using ENV DATADOME_COOKIE + fresh")
    else:
        working_cookies = [fresh_user_cookie]
        print(f"[OTP] Using fresh iPhone cookie")
    ...
    for cookie_val in working_cookies:
        for attempt in range(2):
            try:
                ...
                if resp.status_code == 403 and "captcha" not in resp.text.lower():
                    continue
            except Exception as e:
                print(f"[OTP] Error {e}")
                continue
