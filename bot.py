...
async def ff_ban_token(update, context):
    ...
    try:
        ...
        if not jwt_token:
            await msg.edit_text(f"❌ Authentication Failed!\n\nReason: {error_msg}", reply_markup=get_main_keyboard())
            return ConversationHandler.END
        ...
        try:
            await msg.edit_text(f"❌ Error: {str(e)[:150]}", reply_markup=get_main_keyboard())
        except:
            await update.message.reply_text(f"❌ Error: {str(e)[:150]}", reply_markup=get_main_keyboard())
        return ConversationHandler.END
...
async def ban_check_uid(update, context):
    ...
    try:
        ...
        if response and response.status_code == 200:
            ...
            await msg.edit_text(txt, reply_markup=get_main_keyboard())
        else:
