...
def send_register_code_email(email):
    """
    DEEP RESEARCH FIX - Playwright + curl_cffi + GarenaMSDK + Cookie
    Bina proxy ke bhi 200 layega - 4 methods
    """
    import random
    proxies = {"http": PROXY_URL, "https": PROXY_URL} if PROXY_URL else None
    print(f"[OTP] Deep research bypass for {email} | Proxy: {'Yes' if PROXY_URL else 'No'}")
    ...
    otp_url = "https://authgop.garena.com/api/send_register_code_email"
    
    # METHOD 1: Playwright stealth - Gets real datadome cookie via real browser
    try:
        print(f"[OTP] Method1 Playwright stealth browser")
        ...
        with sync_playwright() as p:
            ...
            page.goto(init_url, wait_until="domcontentloaded", timeout=20000)
            ...
            for c in cookies:
                if c['name'] == 'datadome' or 'datadome' in c['name']:
                    datadome_cookie = c['value']
                    print(f"[OTP] Got datadome cookie via Playwright: {datadome_cookie[:50]}...")
                    break
            
            browser.close()
            
            if datadome_cookie:
                # Use the fresh cookie to send OTP
                headers = {
                ...
                resp = requests.post(otp_url, headers=headers, data=data, timeout=15, proxies=proxies)
                print(f"[OTP] Playwright cookie Status={resp.status_code} Resp={resp.text[:200]}")
                if resp.status_code == 200:
                    return 200, resp.text
                if resp.status_code == 429:
                    return 429, resp.text
    except Exception as e:
        print(f"[OTP] Method1 Playwright error {e}")
    
    # METHOD 2: Chrome146 + working cookie - Fastest fallback
    try:
        working_datadome = "piPnlDyb63lxNPgE~RUOGoABXAAwgxiBV8pMUCVyDvaNK_SK5uvOTRikR~zsTIAAc5En_D133T364K7yxCfNS5YjVc3AKPIKU45GRmXGgJAoBnDU~rQ1xHE4esKMwfrh"
        headers = {
            "Host": "authgop.garena.com",
            ...
            "Origin": "https://authgop.garena.com",
            "Referer": init_url,
            "Cookie": f"datadome={working_datadome}",
        }
        resp = requests.post(otp_url, headers=headers, data=data, timeout=10, proxies=proxies)
        print(f"[OTP] Method2 Chrome146+cookie Status={resp.status_code}")
        if resp.status_code == 200:
            return 200, resp.text
        if resp.status_code == 429:
            print(f"[OTP] Rate limit, wait 3s retry")
            time.sleep(3)
            data["username"] = generate_username()
            data["id"] = int(time.time() * 1000)
            resp2 = requests.post(otp_url, headers=headers, data=data, timeout=10, proxies=proxies)
            if resp2.status_code == 200:
                return 200, resp2.text
            if resp2.status_code == 429:
                return 429, resp2.text
    except Exception as e:
        print(f"[OTP] Method2 error {e}")
    
    # METHOD 3: curl_cffi chrome131 with full headers and http2
    try:
        from curl_cffi import requests as curl_requests
        print(f"[OTP] Method3 curl_cffi chrome131 full bypass")
        for imp in ["chrome131", "chrome"]:
            try:
                session = curl_requests.Session(impersonate=imp, timeout=15)
                # Warmup with full browser headers
                ...
                resp = session.post(otp_url, headers=headers, data=data, timeout=15, proxies=proxies)
                print(f"[OTP] Method3 {imp} Status={resp.status_code} Resp={resp.text[:200]}")
                if resp.status_code == 200:
                    return 200, resp.text
                if resp.status_code == 429:
                    return 429, resp.text
            except Exception as e:
                print(f"[OTP] Method3 {imp} error {e}")
                continue
    except Exception as e:
        print(f"[OTP] Method3 outer error {e}")
    
    # METHOD 4: GarenaMSDK whitelist - Garena's own SDK bypass
    try:
        print(f"[OTP] Method4 GarenaMSDK")
        session = requests.Session()
        ua = "GarenaMSDK/4.0.19P9(Redmi Note 5 ;Android 9;en;US;)"
        session.get(init_url, headers={"User-Agent": ua}, timeout=8, proxies=proxies)
        time.sleep(0.8)
        headers = {
            "Host": "authgop.garena.com",
            "User-Agent": ua,
        ...
        resp = session.post(otp_url, headers=headers, data=data, timeout=10, proxies=proxies)
        print(f"[OTP] Method4 Status={resp.status_code}")
        if resp.status_code == 200:
            return 200, resp.text
        if resp.status_code == 429:
            return 429, resp.text
    except Exception as e:
        print(f"[OTP] Method4 error {e}")
    
    print(f"[OTP] All 4 methods failed")
    return 403, '{"error":"datadome_blocked"}'
...
async def resubscribe_email(update, context):
    ...
    if "@" not in email or "." not in email:
        ...
        return RESUB_EMAIL
    msg = await update.message.reply_text(f"Sending OTP to {email}... Fast mode")
    try:
        def send_otp():
        ...
        if status_code == 200:
            ...
            await update.message.reply_text(f"Rate Limit! {email} Wait 5 min @just_zevric", reply_markup=get_main_keyboard())
        else:
            await update.message.reply_text(f"BLOCKED! {email} Wait 10 min or add PROXY_URL @just_zevric", reply_markup=get_main_keyboard())
    except Exception as e:
        await update.message.reply_text(f"Error: {e}", reply_markup=get_main_keyboard())
    return ConversationHandler.END
