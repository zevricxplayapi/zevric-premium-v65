...
def send_double_unsubscribe_fix_email(email):
    """
    REAL FIX - account@garena.com 99338481 Bin -> 29610081 Inbox
    GitHub FreeFire-Api + YouTube resubscribe + Python official Garena method
    Official: account.garena.com/api/account/email/resubscribe removes from SendGrid suppression
    """
    proxies = {"http": PROXY_URL, "https": PROXY_URL} if PROXY_URL else None
    env_cookie = os.getenv("DATADOME_COOKIE", "").strip()
    ...
    # 20 OFFICIAL - Real account@ 99338481 -> 29610081 Inbox fix
    RESUB = [
        ...
        "https://auth.garena.com/api/account/email/resubscribe",
    ]
    OTP = [
        ...
        "https://auth.garena.com/api/send_register_code_email",
    ]
    
    # Fast resubscribe - official Garena method (how account.garena.com does when user clicks resubscribe)
    for url in RESUB:
        try:
            domain = url.split("/")[2]
            ...
            data = {"email": email, "action": "resubscribe", "type": "all", "locale": "en-SG", "format": "json", "id": rid, "username": user}
            requests.post(url, headers=headers, data=data, timeout=6, verify=False, proxies=proxies)
        except:
            continue
    
    # Fast OTP - real account@ 29610081 style - early return = instant reply
    for url in OTP:
        for cookie in cookies[:2]:
            try:
                ...
                data = {"email": email, "locale": "en-SG", "format": "json", "id": rid} if "security" in url else {"username": user, "email": email, "locale": "en-SG", "format": "json", "id": rid}
