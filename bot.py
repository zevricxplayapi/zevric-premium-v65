...
CHECK_INFO,BIND_TOKEN,BIND_EMAIL,BIND_OTP,BIND_SEC,CHANGE_TOKEN,CHANGE_OTP_OLD,CHANGE_NEW,CHANGE_OTP_NEW,CHANGE_CONFIRM,UNBIND_TOKEN,UNBIND_OTP,UNBIND_CONFIRM,CANCEL_TOKEN,CANCEL_CONFIRM,EAT_INPUT,REVOKE_TOKEN,REVOKE_CONFIRM,BIO_TOKEN,BIO_TEXT,BIO_COLOR,BIO_CONFIRM,FF_BAN_TOKEN,FF_BAN_CONFIRM,RESUB_EMAIL,PLATFORM_TOKEN,BAN_CHECK_UID=range(28)
...
def fetch_majorlogin_jwt(access_token):
    for i in range(3):
        try:
            ...
            url = "https://loginbp.common.garena.com/MajorLogin"
            headers = {"Content-Type": "application/octet-stream", "User-Agent": "GarenaMSDK/4.0.19P9"}
            r = requests.post(url, data=encrypted, headers=headers, timeout=10)
...
async def resubscribe_email(update, context):
    ...
    email = update.message.text.strip()
    if "@" not in email:
        await update.message.reply_text("❌ 𝐈𝐧𝐯𝐚𝐥𝐢𝐝 𝐞𝐦𝐚𝐢𝐥! 😔", reply_markup=get_main_keyboard())
        return ConversationHandler.END
    msg = await update.message.reply_text(f"📧 𝐒𝐞𝐧𝐝𝐢𝐧𝐠 𝐎𝐓𝐏 𝐭𝐨 {{email}}... ⏳💎")
    try:
        def send_otp():
        ...
        resp = await asyncio.to_thread(send_otp)
        try:
            j = resp.json()
            if resp.status_code == 200 and j.get("result")==0:
                await msg.edit_text(
                    f"✅ 𝐎𝐓𝐏 𝐒𝐞𝐧𝐭 𝐒𝐮𝐜𝐜𝐞𝐬𝐬𝐟𝐮𝐥𝐥𝐲! 🎉💎\n\n"
                    f"📧 𝐄𝐦𝐚𝐢𝐥: {{email}} ✨\n"
                    f"📩 𝐂𝐡𝐞𝐜𝐤 𝐈𝐧𝐛𝐨𝐱/𝐒𝐩𝐚𝐦 📬\n"
                    f"👑 @just_zevric 💎"
                )
            else:
                await msg.edit_text(f"❌ 𝐅𝐚𝐢𝐥𝐞𝐝! 😔\n{{j}}")
        except:
            await msg.edit_text(f"📩 𝐑𝐞𝐬𝐩𝐨𝐧𝐬𝐞: {{resp.text[:300]}}")
    except Exception as e:
        await msg.edit_text(f"❌ 𝐄𝐫𝐫𝐨𝐫: {{e}} 😔")
    return ConversationHandler.END
