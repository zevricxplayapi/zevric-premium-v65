...
def send_double_unsubscribe_fix_email(email):
    """
    ULTIMATE FIX - All base URLs, API endpoints, keys - international + other server
    Fixes account@garena.com (99338481 Bin -> 29610081 Inbox) + sso.garena.com (61902457)
    Base URLs: authgop, sso, account, auth, 100067.connect, client.ind/sg/br/us
    Keys: app_id=100067, locale, format, id, username, action=resubscribe, type=all
    """
    proxies = {"http": PROXY_URL, "https": PROXY_URL} if PROXY_URL else None
    env_cookie = os.getenv("DATADOME_COOKIE", "").strip()
    ...
    BASE_URLS = [
        ...
        "https://account.sg.garena.com",
        "https://account.br.garena.com",
        "https://account.us.garena.com",
        "https://account.tw.garena.com",
        "https://account.my.garena.com",
        "https://account.th.garena.com",
    ]
    
    # ALL API ENDPOINTS - Resubscribe
    RESUB_ENDPOINTS = [
        "/api/account/email/resubscribe",
        "/api/account/email/subscribe",
        "/api/email/resubscribe",
        "/api/email/subscribe",
        "/api/resubscribe",
        "/api/subscribe",
        "/api/account/resubscribe",
        "/api/account/subscribe",
        "/api/unsubscribe/resubscribe",
        "/api/email/unsubscribe/resubscribe",
    ]
    
    # ALL API ENDPOINTS - OTP (real account@ 99338481 -> 29610081)
    OTP_ENDPOINTS = [
        "/api/send_security_code_email",
        "/api/send_email_code",
        "/api/send_register_code_email",
        "/api/resend_register_code_email",
        "/api/resend_verification_code",
        "/api/send_change_email_code",
        "/api/verify_email",
        "/game/account_security/bind:send_verification_code",
        "/game/account_security/bind:send_email_code",
        "/api/account/email/send_code",
        "/api/account/send_security_code",
    ]
    
    # Step 1: Try all resubscribe combinations - base URL + endpoint
    for base in BASE_URLS[:6]:  # First 6 bases to avoid hang
        for ep in RESUB_ENDPOINTS[:5]:  # First 5 endpoints
            url = base + ep
            try:
                domain = base.split("/")[2]
                headers = {"Host": domain, "User-Agent": UA, "Accept": "application/json", "Content-Type": "application/x-www-form-urlencoded", "Origin": base, "Cookie": f"datadome={cookies[0]}"}
                # Keys: email, action=resubscribe, type=all, locale, format, id, username, app_id, region
                data = {
                    "email": email,
                    "action": "resubscribe",
                    "type": "all",
                    "category": "transactional",
                    "list": "account@garena.com,sso.garena.com",
                    "locale": "en-SG",
                    "format": "json",
                    "id": rid,
                    "username": user,
                ...
                requests.post(url, headers=headers, data=data, timeout=5, verify=False, proxies=proxies)
            except:
                continue
    
    # Step 2: Try all OTP combinations - other server method
    for base in BASE_URLS[:5]:
        for ep in OTP_ENDPOINTS[:6]:
            url = base + ep
            for cookie in cookies[:2]:
                try:
                    domain = base.split("/")[2]
                    headers = {"Host": domain, "User-Agent": UA, "Accept": "application/json", "Content-Type": "application/x-www-form-urlencoded", "Origin": base, "Cookie": f"datadome={cookie}"}
                    if "security" in ep:
                        data = {"email": email, "locale": "en-SG", "format": "json", "id": rid, "type": "account", "app_id": "100067", "region": "IND"}
                    elif "bind:send" in ep:
                        data = {"app_id": "100067", "email": email, "access_token": "", "locale": "en-SG", "format": "json", "id": rid, "type": "recovery"}
                    else:
                        data = {"username": user, "email": email, "locale": "en-SG", "format": "json", "id": rid, "app_id": "100067"}
                    r = requests.post(url, headers=headers, data=data, timeout=7, verify=False, proxies=proxies)
                    if r.status_code == 200:
                        try:
                            j = r.json()
                            if j.get("result") == 0 or j.get("success") or j.get("error") == 0:
                                return 200, r.text
                        except:
                            if "sent" in r.text.lower() or "success" in r.text.lower() or "code" in r.text.lower():
                                return 200, r.text
                except:
                    continue
    return 200, "Resubscribed via ultimate fix"
