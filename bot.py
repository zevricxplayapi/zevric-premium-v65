...
try:
    ...
    pass
# Fallback: add hardcoded if needed (user can set env)
# ADMIN_IDS = [123456789]  # apna Telegram ID yahan daalo ya env me set karo
...
✅ Screenshot + UTR/TxID bhejo

👑 @just_zevric"""
    
    await update.message.reply_text(txt, reply_markup=get_main_keyboard())
    
    # Send UPI QR - Check both absolute and local
    try:
        upi_found = None
        for p in [UPI_QR_PATH, UPI_QR_PATH_LOCAL, "upi_qr.jpg", "upi_qr.jpeg", "./upi_qr.png"]:
            if os.path.exists(p):
                upi_found = p
                break
        if upi_found:
            ...
            print(f"[PAY] UPI QR not found in any path")
    except Exception as e:
        print(f"[PAY] UPI QR error: {e}")
    
    # Send USDT QR - Check both absolute and local
    try:
        usdt_found = None
        for p in [USDT_QR_PATH, USDT_QR_PATH_LOCAL, USDT_QR_PATH2, "usdt_qr.jpg", "usdt_qr.jpeg", "./usdt_qr.png"]:
            if os.path.exists(p):
                usdt_found = p
                break
        if usdt_found:
...
async def payment_proof_handler(update, context):
    ...
    try:
        ...
        if update.message.text:
            entry["proof_text"] = update.message.text[:500]
            entry["type"] = "text"
        # If photo
        if update.message.photo:
            entry["type"] = "photo"
            entry["proof_text"] = "Photo proof sent"
        
        pending_db[uid_str].append(entry)
        save_pending_db(pending_db)
        
        # Forward to admin if ADMIN_IDS set
...
📝 Proof: {entry.get('proof_text','Photo')[:300]}

✅ Approve: /approve {user_id} {option_key}
❌ Reject: /reject {user_id} {option_key}
...
👤 {opt_info['name']} | ₹{opt_info['price_inr']}
⏳ Admin 5 min me check karega
💬 Fast approval: @just_zevric ko msg karo
📸 TxID/UTR sahi bheja hai na check karo
